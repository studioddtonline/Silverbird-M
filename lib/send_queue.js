...
SendQueue.prototype = {
  enqueueTweet: function(message, replyId, replyUser, isDM, media) {
    if(this._isDuplicate(message)) {
      return;
    }
    var queuedTweet = new QueuedTweet(this.twitterBackend, message, replyId, replyUser, isDM, media);
...
function QueuedTweet(twitterBackend, message, replyId, replyUser, isDM, media) {
  this.twitterBackend = twitterBackend;
