$.extend(Renderer, {
  assemblyTweets: function (tweets, timelineId) {
    ...
    var renderdText = '',
        doAggressive = (!OptionsBackend.get('compliant_twitter_display_requirements') && OptionsBackend.get('aggressive_flat'));
    ...
    .on('mouseover', function(event) {
      ...
      .each(function() {
        if(!this.dataset.userId || !this.dataset.userName) return;
        if(this.dataset.userId !== "1266336019"
        && this.dataset.userId !== 'undefined'
        && this.dataset.userName !== 'undefined') {
          Renderer.createUserActionMenu(this, this.dataset.userId, this.dataset.userName);
        }
        this.removeAttribute("data-user-id");
        this.removeAttribute("data-user-name");
      })
      .end()
      .find('.handleHashTag')
      .each(function() {
    ...
    .find('.tweet')
    .on({
      "mouseover": (function(doAggressive) {
        return function(event) {
          var newActions = $(".new_actions", this), img;
          if(doAggressive) {
            newActions.css('display', 'inline-block');
            img = newActions.find("img");
          } else {
            img = newActions.find("img").not(".starred");
          }
          img.css('display', 'inline');
        };
      })(doAggressive),
      "mouseout": (function(doAggressive) {
        return function(event) {
          var newActions = $(".new_actions", this), img;
          if(doAggressive) {
            newActions.css('display', 'none');
            img = newActions.find("img");
          } else {
            img = newActions.find("img").not(".starred");
          }
    ...
    if(doAggressive) {
      Renderer.applyAggressiveFlatStyle(destination);
    }
    renderdText = null;
    doAggressive = null;
    destination = null;
  ...
  createUserActionMenu: function(element, userId, userName) {
  ...
  applyAggressiveFlatStyle: function(jqElement) {
    if(!jqElement) return;
    jqElement
    .css({paddingRight: ".5em"})
    .find('img')
    .removeClass('profile retweet_source retweet_retweeter verified protected retweet')
    .css({maxWidth: "1em", maxHeight: "1em", borderRadius: ".5em"})
    .end()
    .find('.tweet')
    .css({padding: "0"})
    .end()
    ...
    .end()
    .find(".text_container")
    .css({display: "inline", wordWrap: "break-word", margin: "0", padding: "0"})
    .end()
    .find('.new_actions')
    .css({position: "relative", float: "right", zIndex: "99999", margin: "-.4em .2em 0", border: "solid .2em #646464", borderRadius: ".5em", padding: "0 .2em", backgroundColor: "#f8f8f8", display: "none"})
    .end()
    .find('.header_container') 
    .attr('aggressive', '')
    .end()
    .find('.first_container div')
    .not(".text_container")
    .removeClass()
    .css({display: "inline", wordWrap: "break-word"})
    .end()
    .end()
    .find("[aggressive]")
    .addClass("aggresive_after_content");
  }
});
