...
SendQueue.prototype = {
  enqueueTweet: function(message, replyId, replyUser, media) {
    if(this._isDuplicate(message)) {
      return;
    }
    var queuedTweet = new QueuedTweet(this.twitterBackend, message, replyId, replyUser, media);
  ...
  _sender: function() {
    ...
    closure_function = function(self) {
    ...
    tweetToSend.send(closure_function(this));
  }
};

function QueuedTweet(twitterBackend, message, replyId, replyUser, media) {
  this.twitterBackend = twitterBackend;
...
QueuedTweet.prototype = {
  send: function(callback) {
    ...
    this.retryCount += 1;
    if(this.media) {
      this.twitterBackend.tweet_with_media(callback, this.message, this.replyId, this.media);
    } else {
