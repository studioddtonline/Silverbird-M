...
Object.defineProperties(Expander.prototype, {
  ...
  '_resolve': {
    value: function(data, args, result) {
      ...
      result.url = rURL.split('/').map(function(entry) {
        try {
      ...
      result.mime = this._mime(this._xhr);
      if(args.method == 'GET' && args.responseType == 'document' && data.target.response.documentElement) {
        result.document = data.target.response.documentElement;
      }
      result.content = this._scraping(result) || this._content(result);
      if(result.success) {
  ...
  '_reject': {
    value: function(data, args, result) {
      if((data.target.status == 403 || data.target.status == 405) && args.method == 'HEAD') {
        args.method = 'GET';
        args.force = true;
        this.expand(args);
        return;
  ...
  '_content': {
    value: function(result) {
      var content = undefined, mime = result.mime, url = result.url;
      if(!mime || !url) {
  ...
  '_scraping': {
    value: function(result) {
      ...
      if(!result.document) return content;
      try {
        ...
        if(doc) {
          var image = doc.querySelector('meta[property=twitter\\:image]')
                  ...
                  || doc.querySelector('meta[class=class\\:image]')
                  || doc.querySelector('.animated-gif-thumbnail');
          var thumb = image.getAttribute('content') || image.getAttribute('value') || image.getAttribute('src') || undefined;
          if(thumb) {
            if(this._regexp.lazyRelativePath.test(thumb)) {
              thumb = thumb.replace(this._regexp.lazyRelativePath, 'http://');
            }
  ...
  '_finally': {
    value: function(args, result) {
      this._p = null;
      if(args.method.toUpperCase() == 'HEAD' && this._regexp.matchMimeHtml.test(result.mime)) {
        args.method = 'GET';
