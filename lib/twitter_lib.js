...
TwitterLib.prototype = {
        ...
        $.each(allHeaders, function(index, header) {
          switch( true ) {
            case (/X-Rate-?Limit-Remaining/i).test(header):
              _this.rateLimits[apiName].remaining = header.split(/:\s*/)[1];
              break;
            case (/X-Rate-?Limit-Reset/i).test(header):
              _this.rateLimits[apiName].reset = header.split(/:\s*/)[1];
              break;
            case (/X-Rate-?Limit-Limit/i).test(header):
              _this.rateLimits[apiName].limit = header.split(/:\s*/)[1];
              break;
  ...
  updateWindowHitsLimit: function() {
    ...
    this.ajaxRequest("application/rate_limit_status", function(success, data, status, context, xhr) {
      if(success) {+        $.each(data.resources, function(apiFamilies, api) {
          $.each(api, function(apiEndpoint, apiRateInfo) {
            _this.rateLimits[apiEndpoint.split('/').slice(1, 3).join('/')] = apiRateInfo;
          });
        });
      }
      if(xhr) {
    ...
    }, null, {resources: 'search,statuses,direct_messages,users,favorites,lists'});
  },
