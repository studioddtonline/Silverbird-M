$.extend(Renderer, {
  assemblyTweets: function (tweets, timelineId) {
    ...
    var renderdText = '', pNow = performance.now(), dNow = Date.now(), isSkip = false, tId = timelineId.split('_')[0], tmB = undefined, tmM = undefined;
    ...
    for(var entry of tweets.entries()) {
      ...
      if(isSkip && ((tmB.has(tweetOwner) || tmM.has(tweetOwner)) || (tmB.has(rtOwner) || tmM.has(rtOwner)))) {
        // skip rendering
      } else {
