  ...
  renderTweet: function (tweet, useColors) {
    ...
    if(this.isComplete() && !hiddenFooter) {
      ...
      if(!hiddenGeoInfo && tweet.geo) {
        ...
        footer_content += `<span class="geo_tag"><a class="handleLink tooltip" data-handle-link-base="http://maps.google.com/maps?q=loc:${latStr}" data-tooltip-content="<img src=\'http://maps.google.com/maps/api/staticmap?${mapParam}\' />" href="#"><img src="data:image/gif;base64, "/></a></span>`;
      }
      // from list
    ...
    if(this.isComplete() && !/^Notification/.test(tweet.id)) {
      ...
      if(templateId != TimelineTemplate.RECEIVED_DMS && templateId != TimelineTemplate.SENT_DMS) {
        if(tweet.favorited) {
          newActions_container += `<img class="starred new_actions_item" title="${chrome.i18n.getMessage('unmarkFavorite')}" src="img/star_hover.png" />`;
        } else {
          newActions_container += `<img class="unStarred new_actions_item" title="${chrome.i18n.getMessage('markFavorite')}" src="img/star.png" />`;
        }
      }
      if(selfTweet) {
        var titleStrig;
        ...
        newActions_container += `<img class="destroyIcon new_actions_item" title="${titleString}" src="img/delete.png" />`;
      } else {
        newActions_container += `<img class="replyIcon new_actions_item" title="${chrome.i18n.getMessage("Reply")}" src="img/reply.png" />`;
        if(tweetManager.isRetweet(tweet)) {
          newActions_container += `<img class="destroyRTIcon new_actions_item" title="${chrome.i18n.getMessage("deleteRT")}" src="img/delete.png" />`;
        } else {
          if(templateId != TimelineTemplate.RECEIVED_DMS && templateId != TimelineTemplate.SENT_DMS && !user['protected']) {
            newActions_container += `<img class="retweetIcon new_actions_item" title="${chrome.i18n.getMessage("Retweet")}" src="img/rt.png" />`;
          }
        }
        if(!user['protected']) {
          newActions_container += `<img class="quoteTweetIcon new_actions_item" title="${chrome.i18n.getMessage("quoteTweet")}" src="img/share.png" data-quote-tweet-url="${timestamp_url}" />`;
        }
      }
