const Composer = {
  ...
  init: function() {
    if(tweetManager.composerData.isComposing) {
      Composer.initMessage(tweetManager.composerData.saveMessage, tweetManager.composerData.replyId, tweetManager.composerData.replyUser, tweetManager.composerData.quoteTweetUrl, tweetManager.composerData.mediaIds);
    } else {
      Composer.textareaChanged();
    }
  },

  initMessage: function(message, replyId, replyUser, quoteTweetUrl, mediaIds) {
  ...
  inputFileOnchange: function(event) {
    ...
    if(files.length > 0) {
      ...
      if(pAll.length > 0) {
        ...
        hideLoading(document.getElementById("uploading"));
        Composer.clearLoadedFile();
      }
    }
  },
  ...
  nowUploading: function(status) {
    ...
    if(Composer.uploading === true) {
      $('#attach_button').attr("disabled", "disabled");
    } else {
      $('#attach_button').removeAttr("disabled");
    }
  }
};
