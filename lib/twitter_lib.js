...
TwitterLib.prototype = {
          ...
          if(request && request.readyState == 4) {
            if(request.status == 401) {
              ...
              if(url.match('direct_messages')) {
                var accessLevel = _this.findResponseHeader(request, 'X-Access-Level') || _this.lastAccessLevel;
                if(accessLevel) {
    ...
    .always(function(data, status, request){
      try{
        ...
        for(var i = 0, len = target.length; i < len; i++) {
          var result = _this.findResponseHeader(request, target[i]);
          if(!result) break;
          switch(i) {
            case 0:
        ...
        for(var i = 0, len = mediaTarget.length; i < len && withMedia; i++) {
          var result = _this.findResponseHeader(request, mediaTarget[i]);
          if(!result) break;
          switch(i) {
            case 0:
  ...
  updateWindowHitsLimit: function() {
    ...
    this.ajaxRequest("application/rate_limit_status", function(success, data, status, context, xhr) {
      ...
      if(xhr) {
        var accessLevel = _this.findResponseHeader(xhr, 'X-Access-Level');
    ...
    }, null, {resources: 'search,statuses,direct_messages,users,favorites,lists,blocks,friends,help'});
  },
