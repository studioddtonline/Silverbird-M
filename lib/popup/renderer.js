var Renderer = {
  ...
  parseEntities: function(text, entities, extended_entities, quoted_status, tweetspaceId) {
    ...
    for(let i in entities) {
      for(let [index, v] of entities[i].entries()) {
        if(!v.indices) {
          continue;
        }
  ...
  renderTweet: function (tweet, now, displayOptions) {
    ...
    if(!!tweet.retweeted_status) {
      ...
      if(selfTweet && !tweetManager.isRetweeted(tweet)) {
    ...
    // text_container
    let text_container = `<div class="text_container">${this.parseEntities(text, entities, extended_entities, quoted_status, tweet.silm_tweetSpaceId)}</div>`;
