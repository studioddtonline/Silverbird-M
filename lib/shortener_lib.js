BitLyShortener = {
  shorten: function(longUrl, callback) {
    if(!Shortener.token || Shortener.token === '') {
  ...
  getAccessToken: function(searchString) {
    ...
    if(code) {
      ...
      .always(function() {
        Shortener.tokenRequested = false;
        setTimeout(function() {
...
GooglShortener = {
  ...
  oauthGetRequestToken: function(longUrl, callback){
    Shortener.token = this.oauth_acessor.token = '';
    Shortener.tokenSecret = this.oauth_acessor.tokenSecret = '';
    Shortener.longUrl = longUrl;
    Shortener.callback = callback;
    ...
    this.sendOAuthRequest(message, (function(self) {
      return function(data, status) {
        ...
        Shortener.token = self.oauth_acessor.token = paramMap['oauth_token'];
        Shortener.tokenSecret = self.oauth_acessor.tokenSecret = paramMap['oauth_token_secret'];
        self.oauthAuthorizeToken();
      };
    })(this), (function(self) {
      return function(request, status, error) {
        Shortener.callback(-1, 'Error Get Request Token: ' + request.statusText + '(' + request.responseText + ')');
      };
    })(this));
  },
  ...
  getAccessToken: function(searchString) {
    ...
    this.sendOAuthRequest(message, (function(self) {
      return function(data, status) {
        ...
        self.sendRequest(Shortener.longUrl, Shortener.callback);
  ...
  oauth_acessor: {
    ...
    tokenSecret: Shortener.tokenSecret,
...
function Shortener(backendId) {
  ...
  this.token = Shortener.token = OptionsBackend.get('shortener_token') || '';
  this.tokenSecret = Shortener.tokenSecret = OptionsBackend.get('shortener_token_secret') || '';
}
