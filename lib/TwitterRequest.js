...
class TwitterUpload extends TwitterRequest {
  ...
  async uploadMedia(file, callback) {
    this.callback = callback;
    if(!file || !file.size || !file.type) {
      this.callback("missing file");
    }
    try {
      const json = await this.uploadMediaInit(file);
      if(!json) {
      ...
      this.callback(await this.uploadMediaFinalize(mediaId));
    } catch(e) {
      console.warn(e);
      return false;
    }
  }
  async uploadMediaInit(file) {
