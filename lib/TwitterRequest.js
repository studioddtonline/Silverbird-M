  ...
  async fetchRequest(request) {
    ...
    try {
      ...
      this.streamReader.closed.then(() => {
        ...
        if(this.state) {
          console.log("UserStream: Uncaught Disconnect");
          this.disconnect();
        }
        return Promise.resolve();
      }).catch((e) => {
      ...
      console.log("UserStream.fetchRequest: %s", e);
      this.disconnect();
    }
  }
  async readStream() {
    ...
    try {
      const result = await this.streamReader.read() || {value: new Uint8Array()};
      this.processStream(this.decoder.decode(result.value));
      if(!!result.done) {
        throw new TypeError("UserStream: Uncaught Done");
      }
