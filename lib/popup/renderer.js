$.extend(Renderer, {
  assemblyTweets: function (tweets, timelineId) {
    ...
    $('.text_container').one('mouseover', function(event) {
      $(this)
      .find('.handleLink')
      .each(function() {
  ...
  handleLink: function(link, baseUrl, expandedUrl, mediaUrl) {
    ...
    if(!OptionsBackend.get('show_expanded_urls') || link.dataset.handleLinkNoexpand == 'true') {
      link.setAttribute('data-tooltip-link', toExpandUrl);
      return;
    }
    ...
    tweetManager.urlExpander.expand(toExpandUrl, function expanded(success, isShortened, longUrl) {
      ...
      if(!isShortened) {
        ...
        if(reCheck) {
          ...
          changeContent = longUrl;
        } else {
          changeContent = chrome.i18n.getMessage("errorExpandingUrl");
        }
      }
      try {
        changeContent = decodeURIComponent(changeContent);
      } catch(e) {
        // no behavior
      }
