  ...
  handleLink: function(link, baseUrl, expandedUrl, mediaUrl) {
    ...
    tweetManager.urlExpander.expand({
      ...
      callback: function(result) {
        if(result.content) {
          link.setAttribute('data-tooltip-content', result.content);
        } else {
          var reCheck = Renderer.expandImageLink(result.url);
          if(reCheck) {
            link.setAttribute('data-tooltip-content', '<img src="' + reCheck + '" />');
          } else {
            link.setAttribute('data-tooltip-content', '<p>' + result.url + '</p>');
          }
        }
      }
    });
