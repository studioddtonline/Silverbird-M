...
Expander.prototype = {
  ...
  updateServicesCache: function(service) {
    ...
    this.servicesTimer = null;
    var _this = this, result = service.services(), url = result[0], params = result[1], dataType = result[2];
  ...
  runExpander: function(service, shortUrl, callback) {
    var _this = this, result = service.expand(shortUrl), url = result[0], params = result[1], dataType = result[2];
    this.doAjaxRequest(url, params, dataType,
      function(data, status, request) {
        ...
        if(_this.services[longUrlDomain]) {
          ...
          _this.expand(longUrl, callback, _this.loopRemaining);
        } else {
          callback(success, isShortened, longUrl);
        }
        _this = null;
