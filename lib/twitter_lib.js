...
TwitterLib.prototype = {
  ...
  ajaxRequest: function(url, callback, context, requestParams, httpMethod, authType, overriddenTimeout, withMedia) {
    ...
    var beforeSendCallback = function(request, settings) {
      ...
      rateLimits.last = now;
      _this.signOauth(request, signingUrl, requestParams, httpMethod, authType);
    };
          ...
          if(request && request.readyState == 4) {
            if(request.status == 401) {
              if(_this.oauthLib.adjustTimestamp(request, 'Date')) {
                ...
                _this.ajaxRequest(url, callback, context, requestParams, httpMethod, authType, overriddenTimeout);
                return;
              ...
              } else if(url.match('update_with_media')) {
    ...
    .always(function(data, status, request){
      try {
        ...
        for(var i = 0; i < len; i++) {
          ...
          switch(true) {
            case (/X-RateLimit-Remaining/i).test(header):
              rateLimits.remaining = parseInt(header.split(/:\s*/)[1], 10);
              break;
            case (/X-RateLimit-Reset/i).test(header):
              rateLimits.reset = parseInt(header.split(/:\s*/)[1], 10) * 1000;
              break;
            case (/X-RateLimit-Limit/i).test(header):
              rateLimits.limit = parseInt(header.split(/:\s*/)[1], 10);
              break;
            default:
              continue;
          }
        ...
        //TODO MediaRateLimit headers should be controlled.
        for(var i = 0; i < len && withMedia; i++) {
          var header = allHeaders[i];
          switch(true) {
            case (/X-MediaRateLimit-Class/i).test(header):
              if(console) console.log('X-MediaRateLimit-Class: ' + header.split(/:\s*/)[1]);
              break;
            case (/X-MediaRateLimit-Remaining/i).test(header):
              if(console) console.log('X-MediaRateLimit-Remaining: ' + parseInt(header.split(/:\s*/)[1], 10));
              break;
            case (/X-MediaRateLimit-Reset/i).test(header):
              if(console) console.log('X-MediaRateLimit-Reset: ' + new Date((parseInt(header.split(/:\s*/)[1], 10) * 1000)).toLocaleString());
              break;
            case (/X-MediaRateLimit-Limit/i).test(header):
              if(console) console.log('X-MediaRateLimit-Limit: ' + parseInt(header.split(/:\s*/)[1], 10));
              break;
            default:
              continue;
          }
