var Renderer = {
  ...
  buildIcon: function(tweet, isRetweet = true, displayOptions = {hiddenUserIcons: false, iconSize: "icon_normal"}) {
    ...
    if(!displayOptions.hiddenUserIcons) {
      ...
      ret = `<img data-user-id="${user.id_str}" data-user-name="${user.screen_name}" class="createUserActionMenu profile ${iconStyle}" src="${user.profile_image_url_https.replace(/_(mini|normal|bigger)\./, iconSize)}" />`;
    }
    return ret;
  },
