...
var Renderer = {
  ...
  renderTweet: function (tweet, now, useColors) {
    ...
    // new_actions
    if(this.isComplete() && !/^Notification/.test(tweet.id_str)) {
    ...
    return `<div class="tweet_space" id="${tweetspaceId}"><div class="chromed_bird_tweet tweet" timelineid="${tweetTimeline}" tweetid="${tweet.id_str}"><div class="tweet_overlay" style="${overlayStyle}"><div class="first_container">${profile_container}${header_container}${text_container}${footer_container}</div>${newActions_container}</div></div></div>`;
  }
};
