...
Expander.prototype = {
  ...
  updateServicesCache: function(service) {
    ...
    this.doAjaxRequest(url, params, dataType, closure_succsss(this), closure_error(this));
  },
  ...
  runExpander: function(service, shortUrl, callback) {
    ...
    var closure_error = function(self) {
      return function(request, status, error) {
        var success = false, isShortened = true;
        callback(success, isShortened, '"' + error + '"(' + status + ')');
      };
    };
    this.doAjaxRequest(url, params, dataType, closure_success(this), closure_error(this));
  }
};
