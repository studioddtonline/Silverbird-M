...
var Renderer = {
  ...
  renderTweet: function (tweet, useColors, nameAttribute) {
    ...
    if(nameAttribute == "both") {
      ...
      var secondaryNameDiv;
      if(Renderer.isOnPage()) {
        secondaryNameDiv = primaryNameDiv;
      } else {
        secondaryNameDiv = cloneBaseDiv.clone()
    ...
    if(this.isComplete()) {
      ...
      if(tweet.in_reply_to_status_id) {
        ...
        var inReply = cloneBaseAnchor.clone().text(tweet.in_reply_to_screen_name);
      ...
      if(templateId == TimelineTemplate.LISTS && tweetManager.currentTimelineId != tweetTimeline) {
        ...
        if(list !== null) {
          ...
          cloneBaseSpan.clone()
            .addClass('from_list')
            .append(Renderer.makeText('(' + chrome.i18n.getMessage("f_footer_list") + ': '))
            .append(listLink)
