...
function updateNotificationFunc(timeline) {
  ...
  const [count, unreadCount] = tweetManager.newTweetsCount(timelineId);
  ...
  if(timeline.timelineId == tweetManager.currentTimelineId) {
    ...
    newTweetsAvailable(count, unreadCount, timelineId);
  }
}
...
function bindEvents() {
  ...
  .on({
    'updateTimeline': function(event) {
      ...
      newTweetsAvailable(event.detail.count, event.detail.unreadCount, event.detail.timelineId);
    },
