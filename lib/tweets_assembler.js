...
var Renderer = {
  ...
  parseEntities: function(text, entities) {
    ...
    for (i = 0, len = orderedEntities.length; i < len; ++i) {
      ...
      if (entity.type === 'mention') {
        ...
        link = $(d.createElement('a')).attr('href', '#').html(entity.screen_name);
        Renderer.createUserActionMenu(link, entity.screen_name);
      } else if (entity.type === 'hashtag') {
        link = $(d.createElement('a')).attr('href', '#').html('#' + entity.text);
        ...
        link = $(d.createElement('a')).attr('href', entity.url).html(entity.display_url);
  ...
  renderTweet: function (tweet, useColors) {
    ...
    if(nameAttribute == "both") {
      userName = '<div class="primary_name"><a href="#" class="user" screen_name="' + user.screen_name + '">' + user.name + '</a>' + userVerified + userProtected + bothContent + '<a href="#" class="user" screen_name="' + user.screen_name + '">@' + user.screen_name + '</a></div>';
    ...
    if(nameAttribute == "both" && !displaySimpleName) {
