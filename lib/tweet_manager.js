      ...
      try {
        ...
        for(i = 0, len = tweetsToNotify.length; i < len; ++i) {
          ...
          try {
            ...
            var notificationImage = user.profile_image_url.replace(/_normal.jpg$/, '.jpg');
            var notification = chrome.notifications.create('__Silverbird_M__', {
              type: 'basic',
              ...
              setTimeout(function() {
                chrome.notifications.clear(notificationId, function(wasCleared) {
                  if(!wasCleared && console) console.warn('Notification was not cleared.');
                });
...
function checkVersionChanges(manifest) {
  try {
    ...
    if(compareVersions(currentVersion, storageVersion) !== 0) {
      ...
      } else if(compareVersions(storageVersion, [0, 5, 2, 17]) <= 0) {
