var UploadManager = {
  upload: function(file) {
    ...
    imageService.upload(file, closure_success(this), closure_error(this));
  },
...
function ImageService(domain, options) {
  ...
  this.upload = function(file, onFinish, onProgress) {
    ...
    var closure_function = function(self) {
      return function() {
    ...
    xhr.onreadystatechange = closure_function(this);
...
ImageService.addService('img.ly', {
  thumb: 'http://img.ly/show/thumb/$1'
});

ImageService.addService('picplz.com', {
  thumb: 'http://picplz.com/$1/thumb/'
});

ImageService.addService('instagr.am', {
  thumb: function(path, url) {
    return 'http://' + url.split('/').slice(2, 5).join('/') + '/media/?size=m';
  }
});

ImageService.addService('instagram.com', {
  thumb: function(path, url) {
    return 'http://' + url.split('/').slice(2, 5).join('/') + '/media/?size=m';
  }
});
...
ImageService.addService('img.skitch.com', {
  thumb: function(path, url) {
    var parts = url.match(/^(.+)\.(.+)$/);
    return parts[1] + '.preview.' + parts[2];
  }
});

ImageService.addService('lockerz.com', {
  thumb: 'http://api.plixi.com/api/tpapi.svc/imagefromurl?url=$2&size=medium'
});
...
ImageService.addService('p.twipple.jp', {
  thumb: 'http://p.twpl.jp/show/large/$1'
});

ImageService.addService('img.fxc.am', {
  thumb: '$2'
});

ImageService.addService('www.youtube.com', {
  thumb: function(path, url) {
    for(var i = 0, splitPath = path.split(/[\&\?\#]/), len = splitPath.length; i < len; i++) {
      var k = splitPath[i].split('=');
      if(k[0] == 'v') {
        return 'http://img.youtube.com/vi/' + k[1] + '/0.jpg';
      }
    }
    return undefined;
  }
...
ImageService.addService('www.nicovideo.jp', niconico);
ImageService.addService('nico.ms', niconico);

ImageService.addService('ow.ly', {
  thumb: function(path, url) {
    if(path.split('')[0] != 'i') return undefined;
    var id = path.split('/').pop();
    return 'http://static.ow.ly/photos/original/'+id+'.jpg';
  }
});

ImageService.addService('photozou.jp', {
  thumb: function(path, url) {
    var id = path.split('/').pop();
    return 'http://photozou.jp/p/img/'+id;
  }
});
