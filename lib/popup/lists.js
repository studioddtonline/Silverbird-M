  ...
  init: function(force) {
    ...
    tweetManager.lists(force, function(success, lists, status) {
      ...
      tweetManager.eachTimeline(function(timeline) {
        ...
        var $listSelect = $(["select#", timeline.timelineId, "-selector"].join(''));
        if($listSelect.length === 0) {
          var pos = tweetManager.getTimelinePosition(timeline.timelineId);
          if(pos == -1) {
            pos = undefined;
          }
          TimelineTab.addTab(timeline.timelineId, ['<select id="', timeline.timelineId, '-selector"></select>'].join(''), pos);
          $listSelect = $(["select#", timeline.timelineId, "-selector"].join(''));
        }
        var baseOpEl = $(document.createElement("option"));
        $listSelect.empty();
        $listSelect.append(baseOpEl.clone().val(_this.selector_value).text(chrome.i18n.getMessage("selectList")));
        for(var j = 0; j < lists.length; ++j) {
          $listSelect.append(baseOpEl.clone().val(lists[j].uri).text(lists[j].name));
        }
        $listSelect.append(baseOpEl.clone().val(_this.update_value).text(chrome.i18n.getMessage("updateLists")));
        baseOpEl = null;
