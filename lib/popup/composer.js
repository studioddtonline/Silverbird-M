var Composer = {
  ...
  bindEvents: function() {
    ...
    baseElement.find("textarea").on("keyup blur", Composer.textareaChanged.bind(Composer));
    baseElement.find(".footer #tweetit").on("click", Composer.sendTweet.bind(Composer));
    baseElement.find(".footer #image_input").on("change", ImageUpload.upload.bind(ImageUpload));

    composeTrigger.on("click", function() {
      Composer.showComposeArea();
    });

    shortenerArea.find("input").on("focus", Shortener.focus.bind(Shortener));
    shortenerArea.find("input").on("keyup", Shortener.changed.bind(Shortener));
    shortenerArea.find("input").on("blur", Shortener.blur.bind(Shortener));
    shortenerArea.find("#shorten_current").on("click", Shortener.shortenCurrentPage.bind(Shortener));
    shortenerArea.find("#shortener_button").on("click", function() {
      Shortener.shortenIt();
    });
