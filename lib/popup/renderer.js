const Renderer = {
      ...
      if(change.intersectionRatio === 0) {
        ...
        if(!!tweets) {
          tweets.forEach((el) => {
            ...
            const dataCreatedAt = el.querySelectorAll("[data-created-at]");
            if(!!dataCreatedAt) {
  ...
  parseEntities: function(tweet, isRetweet = true) {
    ...
    const isSuspicious = !!baseTweet.from && baseTweet.from === "stream";
    for(let i in baseTweet.entities) {
      let entity = baseTweet.entities[i];
      switch(i) {
        case "user_mentions":
          ret = Renderer.parseUserMentionsEntity(ret, entity, isSuspicious);
          break;
        case "hashtags":
          ret = Renderer.parseHashtagsEntity(ret, entity, isSuspicious);
          break;
        case "urls":
          ret = Renderer.parseUrlsEntity(ret, entity, tweet.silm_tweetSpaceId, baseTweet.quoted_status, isSuspicious);
          break;
        case "media":
          ret = Renderer.parseMediaEntity(ret, entity, baseTweet.extended_entities, isSuspicious);
          break;
        case "symbols":
        //  ret = Renderer.parseSymbolsEntity(ret, entity, isSuspicious);
          break;
        default:
  ...
  parseUserMentionsEntity: function(texts = [], entity = [], isSuspicious = false) {
    let ret = texts;
    entity.forEach((item) => {
      if(!!item.indices || Array.isArray(item.indices)) {
        ...
        if(isSuspicious && !/^[@＠]/.test(suspiciousTarget)) {
          console.info(`broken usermention item.screen_name: %s`, suspiciousTarget);
        }
  ...
  parseHashtagsEntity: function(texts = [], entity = [], isSuspicious = false) {
    let ret = texts;
    entity.forEach((item) => {
      if(!!item.indices || Array.isArray(item.indices)) {
        const [indexStart, indexEnd] = item.indices;
        const suspiciousTarget = ret.slice(indexStart, indexEnd).join("");
        if(isSuspicious && !/^[#＃]/.test(suspiciousTarget)) {
          console.info(`broken hashtag item.text: %s`, suspiciousTarget);
        }
  ...
  parseUrlsEntity: function(texts = [], entity = [], tweetSpaceId = "", quotedStatus = {id_str: null}, isSuspicious = false) {
    let ret = texts;
    ...
    entity.forEach((item) => {
      if(!!item.indices || Array.isArray(item.indices)) {
        const [indexStart, indexEnd] = item.indices;
        const suspiciousTarget = ret.slice(indexStart, indexEnd).join("");
        if(isSuspicious && !/^https?/.test(suspiciousTarget)) {
          console.info(`broken url item.indices: %s`, suspiciousTarget);
          ret.splice(indexStart, 0, ...item.url);
        }
  ...
  parseMediaEntity: function(texts = [], entity = [], extendedEntities = null, isSuspicious = false) {
    let ret = texts;
    entity.forEach((item) => {
      if(!!item.indices || Array.isArray(item.indices)) {
        const [indexStart, indexEnd] = item.indices;
        const suspiciousTarget = ret.slice(indexStart, indexEnd).join("");
        if(isSuspicious && !/^https?/.test(suspiciousTarget) && suspiciousTarget === "") {
          console.info(`broken media item.indices: %s`, suspiciousTarget);
          ret.splice(indexStart, 0, ...item.url);
        }
        const spliceLength = indexEnd - indexStart;
        const spliceArray = new Array(spliceLength).fill("");
  ...
  parseSymbolsEntity: function(texts = [], entity = [], isSuspicious = false) {
    return texts;
  },
