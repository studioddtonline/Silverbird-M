...
function TweetManager() {
  ...
  Object.defineProperties(this, {
    ...
    "listsCache": {
      get: ((self) => {
        return () => {
          return self.__listsCache || [];
        };
      })(this),
      set: ((self) => {
        return (lists) => {
          if(Array.isArray(lists)) {
            self.__listsCache = lists;
            self.dispatchPopupEvent("updateLists", lists);
          } else {
            throw new TypeError('listsCache must be Array');
          }
        };
      })(this)
    },
    "__savedSearchCache": { // Array
      value: [],
      writable: true
    },
    "savedSearchCache": {
      get: ((self) => {
        return () => {
          return self.__savedSearchCache || [];
        };
      })(this),
      set: ((self) => {
        return (savedSearches) => {
          if(Array.isArray(savedSearches)) {
            self.__savedSearchCache = savedSearches;
            self.dispatchPopupEvent("updateSavedSearch", savedSearches);
          } else {
            throw new TypeError('savedSearchCache must be Array');
          }
        };
      })(this)
    },
    "__cachedTrendingTopics": { // Object or undefined
      value: undefined,
      writable: true
    },
    "cachedTrendingTopics": {
      get: ((self) => {
        return () => {
          return self.__cachedTrendingTopics || {
      ...
      set: ((self) => {
        return (trendingTopics) => {
          if(!Array.isArray(trendingTopics.trends)) {
            self.__cachedTrendingTopics = undefined;
            return;
          }
          if(OptionsBackend.get('unified_visible')
          && OptionsBackend.get('notification_include_unified')
          && OptionsBackend.get('notify_update_trends')) {
            var oldTrends = (self.__cachedTrendingTopics || self.cachedTrendingTopics).trends.slice(0, 10);
            var newTrends = (trendingTopics.trends || []).slice(0, 10);
            self.createUpdateTrendsNotification(newTrends.filter(function(entity) {
              return oldTrends.reduce(function(prev, current, index, arr) {
                return prev && current.name !== entity.name;
              }, true);
            }));
          }
          self.__cachedTrendingTopics = trendingTopics;
          self.dispatchPopupEvent("updateTrendingTopics", trendingTopics);
        };
      })(this)
    }
  });
...
TweetManager.prototype = {
  ...
  isSavedSearch: function(query) {
    var result = -1;
    this.savedSearchCache.forEach((savedSearch, i) => {
