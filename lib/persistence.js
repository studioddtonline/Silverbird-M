...
Persistence = {
  ...
  init: function() {
    ...
    var existingKeys = ['options', 'selected_lists', 'timeline_order', 'oauth_token_data',
      'oauth_token_service', 'version', 'popup_size', 'window_position', 'bearer_token_data'];
    ...
    for(var i = 0, len = existingKeys.length; i < len; ++i) {
      ...
      var closure_function = function(self, key) {
        return function() {
          return self.load(key);
        };
      };
      this[methodName] = closure_function(this, currentKey);
    }
  },
