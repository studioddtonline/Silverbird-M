var Renderer = {
  ...
  renderTweet: function (tweet, now, displayOptions) {
    ...
    if(!displayOptions.hiddenFooter) {
      ...
      // retweet
      if(!displayOptions.hiddenRetweetInfo && (tweet.retweeted_status || tweetManager.isRetweet(tweet))) {
        if(selfTweet) {
    ...
    if(!/^Notification/.test(tweet.id_str)) {
      ...
      if(selfTweet) {
        ...
        if(!hereIsDM && !tweetManager.isRetweet(tweet)) {
        ...
        } else if(tweetManager.isRetweet(tweet) && templateId == TimelineTemplate.MENTIONS) {
          newActions_container += `<span class="glyphicon glyphicon-remove-circle new_actions_item action_delete_tweet" title="${chrome.i18n.getMessage("deleteRT")}" data-delete-target-id="${tweet.id_str}" data-timeline-id="${tweetTimeline}"></span>`;
        }
        if(!hereIsDM) {
        ...
        if(tweetManager.isRetweet(tweet)) {
          ...
          if(!hereIsDM && !user['protected']) {
            ...
            newActions_container += `<span class="glyphicon glyphicon-trash new_actions_item action_delete_tweet" title="${chrome.i18n.getMessage("delete")}" data-delete-target-id="${tweet.id_str}" data-timeline-id="${tweetTimeline}"></span>`;
          }
        }
        if(!hereIsDM && !user['protected']) {
          newActions_container += `<span class="glyphicon glyphicon-comment new_actions_item action_quote" title="${chrome.i18n.getMessage("quoteTweet")}" data-quote-tweet-url="${timestamp_url}"></span>`;
        }
        if(!tweet.retweeted_status && !hereIsDM && (user['allow_dms_from'] === 'everyone' || tweetManager.getFollowersIdsSet().has(user['id_str']))) {
  ...
  assemblyTweets: function (tweets, timelineId) {
    ...
    for(let [index, tweet] of tweets.entries()) {
      var tweetOwner = tweet.user.id_str, rtOwner = tweet.retweeted_status ? tweet.retweeted_status.user.id_str : undefined;
      if(isSkipBM
      && (tmB.has(tweetOwner) || tmM.has(tweetOwner) || tmB.has(rtOwner) || tmM.has(rtOwner))
      ) {
        // skip rendering
      } else if(isSkipRT && tweet.retweeted_status) {
        // skip rendering
      } else {
