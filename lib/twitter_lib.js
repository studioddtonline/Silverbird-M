...
TwitterLib.prototype = {
              ...
              case (request.status === 401):
                switch(true) {
                  ...
                  case (/user_timeline/.test(url)):
                    ...
                    if(console) console.warn('This user is protected, or deleted');
                    break;
                  default:
                ...
                switch(true) {
                  ...
                  case (rspObj.code == 150):
                    if(/direct_messages/.test(url)) {
                      fmtError = rspObj.code;
                      if(console) console.warn('You cannot send messages to users who are not following you.');
                    } else {
                      fmtError = "(" + rspObj.message + ")";
                      if(console) console.warn(request.responseText);
                    }
                  ...
                  case (rspObj.code == 354):
                    if(/direct_messages/.test(url)) {
                      fmtError = rspObj.code;
                      if(console) console.warn('The text of your direct message is over the max character limit.');
                    } else {
                      fmtError = "(" + rspObj.message + ")";
                      if(console) console.warn(request.responseText);
                    }
  ...
  follow: function(callback, userId) {
    if(!userId) {
  ...
  unfollow: function(callback, userId) {
    if(!userId) {
      callback(false, null, "Missing user_id");
      return;
    }
  ...
  report: function(callback, userId) {
    if(!userId) {
      callback(false, null, "Missing user_id");
      return;
    }
  ...
  createMutes: function(callback, userId) {
    if(!userId) {
      callback(false, null, "Missing user_id");
      return;
    }
  ...
  destroyMutes: function(callback, userId) {
    if(!userId) {
      callback(false, null, "Missing user_id");
      return;
    }
  ...
  followers: function(callback, cursor) {
    if(!cursor) cursor = "-1";
    var params = {
      cursor: cursor,
      stringify_ids: 'true',
