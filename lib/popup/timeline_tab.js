var TimelineTab = {
  ...
  addTab: function(timelineId, tabName, pos) {
    var insertTabEl = '<li id="tab_\#timeline-' + timelineId + '" class="timeline_tab"><a href="\#timeline-' + timelineId + '"><span>' + tabName + '</span></a></li>';
    if($.isNumeric(pos) && pos < $("#tabs .ui-tabs-nav li.timeline_tab").length) {
      $("#tabs .ui-tabs-nav li.timeline_tab:eq(" + pos + ")").before(insertTabEl);
    } else {
      $("#tabs .ui-tabs-nav").append(insertTabEl);
    }
    var panelEl = '<div class="timeline" id="timeline-' + timelineId + '"><div class="inner_timeline"></div></div>';
    if($.isNumeric(pos) && pos < $("#tabs .timeline").length) {
      $("#tabs .timeline:eq(" + pos + ")").before(panelEl);
    } else {  
      $("#tabs").append(panelEl);
    }
    $("#tabs").tabs('refresh');
