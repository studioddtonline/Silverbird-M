...
ImageService.addService('instagr.am', {
  thumb: function(path, url) { return url + (url[url.length - 1] == '/' ? '' : '/') + 'media?size=m' }
});
...
ImageService.addService('www.youtube.com', {
  thumb: function(path, url) {
    for(var i = 0, splitPath = path.split(/[\&\?\#]/), j = splitPath.length; i < j; i++) {
...
ImageService.addService('www.nicovideo.jp', {
  thumb: function(path, url) {
    if(path.match(/[ns][m](\d+)/)) {
      var id = RegExp.$1;
      var host = parseInt(id, 10) % 4 + 1;
      return ["http://tn-skr", host, ".smilevideo.jp/smile?i=", id].join('')
    } else return undefined;
  }
});

ImageService.addService('nico.ms', {
  thumb: function(path, url) {
    if(path.match(/[ns][m](\d+)/)) {
      var id = RegExp.$1;
      var host = parseInt(id, 10) % 4 + 1;
