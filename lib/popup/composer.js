var Composer = {
  ...
  textareaChanged: function (e) {
    ...
    if(/^d [a-zA-Z0-9_]+? /.test(str) && OptionsBackend.get('image_upload_service') === 'pic.twitter.com') {
      $('#image_input').val('').attr("disabled", "disabled");
    } else {
      $('#image_input').removeAttr("disabled");
    }
    var availableChars = MAX_TWEET_SIZE - str.length - this.picTwitterCom;
  ...
  sendTweet: function () {
    ...
    if(OptionsBackend.get('image_upload_service') === 'pic.twitter.com') {
      tweetManager.enqueueTweet(textarea.val(), Composer.replyId, Composer.replyUser, document.getElementById('image_input').files[0]);
    } else {
