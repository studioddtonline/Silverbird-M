...
(function($) {
  ...
  $.actionMenu.prototype = {
    ...
    reloadOptions: function() {
      ...
      this.actionAreaEl = divEl.clone().addClass('action_area');
      this.secondLevelAreaEl = divEl.clone().addClass('second_level');
      this.secondLevelTrigger = divEl.clone().addClass('second_level_trigger');
      this.secondLevelTrigger.data('opened', false);

      this.balloon = divEl.clone().addClass('balloon');
      ...
      if(actionsLen === 0) {
        this.actionAreaEl.append(imgEl.clone().addClass('action_loading').attr('src', this.options.loading));
      }
      ...
      for(var i = 0; i < actionsLen; ++i) {
        (function(action) {
          action.actionEl = divEl.clone().addClass('action');
          action.actionEl.anyClick(function(event) {
            return action.action(event);
          });
          if(action.icon) {
            var iconEl = imgEl.clone().attr('src', action.icon).attr('title', action.name);
            action.actionEl.append(iconEl);
          } else {
            var nameEl = aEl.clone().attr('href', '#').text(action.name);
