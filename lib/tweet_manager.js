...
function TweetManager() {
  ...
  Object.defineProperties(this, {
    ...
    "cachedListsOwnerships": {
      get: () => {
        return this[Symbol.for("listsOwnerships")] || this.cachedListsOwnershipsData.val() || [];
      },
      set: (lists = []) => {
    ...
    "cachedListsSubscriptions": {
      get: () => {
        return this[Symbol.for("listsSubscriptions")] || this.cachedListsSubscriptionsData.val() || [];
      },
      set: (lists = []) => {
        if(Array.isArray(lists)) {
    ...
    "cachedSavedSearches": {
      get: () => {
        return this[Symbol.for("savedSearches")] || this.cachedSavedSearchesData.val() || [];
      },
      set: (savedSearches = []) => {
    ...
    [Symbol.for("followingIds")]: { // Array
      value: undefined,
      writable: true
    },
    "cachedFollowingIds": {
      get: () => {
        return this[Symbol.for("followingIds")] || this.cachedFollowingIdsData.val() || [];
      },
      set: (followingIds = []) => {
    ...
    "cachedFollowingNames": {
      get: () => {
        return this[Symbol.for("followingNames")] || this.cachedFollowingNamesData.val() || [];
      },
      set: (followingNames = []) => {
  ...
  this.twitterBackend = new TwitterLib(
    (success, data) => {
      ...
      if(this.cachedFollowingIds.length !== data.friends_count || this.cachedFollowingNames.length === 0) {
...
TweetManager.prototype = {
  ...
  onStreamData: function(data) {
    ...
    if(data.friends_str && Array.isArray(data.friends_str)) {
      if(this.cachedFollowingIds.length < data.friends_str.length) {
        this.cachedFollowingIds = data.friends_str;
      }
      return;
    }
    if(data.event) {
      ...
      switch(data.event) {
        ...
        case 'mute':
          ...
          return; // do not notification
        case 'unmute':
          this.mutingIdsSet.delete(data.target.id_str);
          this.resetTimeline(TimelineTemplate.HOME);
          this.resetTimeline(TimelineTemplate.UNIFIED);
          return; // do not notification
        case 'like':
  ...
  retrieveFollowingUsers: function(context = {cursor: "-1", current: []}) {
    ...
    return (new Promise((resolve, reject) => {
      ...
      this.cachedFollowingIds = context.current || [];
      return Promise.resolve();
    }).catch((status) => {
      console.log(status);
      return Promise.reject();
  ...
  lookupFollowingUsers: function(context = {lookupTargets: [], current: []}) {
    ...
    if(context.lookupTargets.length === 0) {
      let len = Math.ceil(this.cachedFollowingIds.length / 100.0);
      if(len > 14) {
        console.warn("too match following targets.");
        len = 14;
      }
      for(let i = 0; i < len; ++i) {
        context.lookupTargets.push(this.cachedFollowingIds.slice((i * 100), (i * 100) + 100));
      }
    }
    return (new Promise((resolve, reject) => {
  ...
  isFollowing: function(id = "0") {
    return this.cachedFollowingIds.includes(id);
  },
  ...
  _removeUser: function(context) {
    if(context.unfollow || context.blocked) {
      this.cachedFollowingIds = this.cachedFollowingIds.filter((entry) => {
        return entry !== context.userData.id_str;
      });
  ...
  _addFollowingUser: function(userData) {
    ...
    if(!this.cachedFollowingIds.includes(userData.id_str)) {
      this.cachedFollowingIds = this.cachedFollowingIds.concat(userData.id_str);
    }
    this.twitterBackend.userData.friends_count += 1;
    this.resetTimeline(TimelineTemplate.HOME);
    this.resetTimeline(TimelineTemplate.UNIFIED);
