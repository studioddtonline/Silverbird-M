...
function TweetManager() {
  ...
  this.twitterBackend = new TwitterLib(
    (success, data) => {
      ...
      if(this.cachedFollowingIds.length !== data.friends_count) {
...
TweetManager.prototype = {
  ...
  lookupFollowingUsers: function(D, lookupTargets) {
    ...
    if(!lookupTargets) {
      this.followingUsersNames = [];
      lookupTargets = [];
      for(var i = 0, len = Math.ceil(this.cachedFollowingIds.length / 100.0); i < len; ++i) {
        var index = i * 100;
        lookupTargets.push(this.cachedFollowingIds.slice(index, index + 100));
      }
    }
