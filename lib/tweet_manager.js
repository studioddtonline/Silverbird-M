...
TweetManager.prototype = {
  ...
  setInfoToPopup: function(msg) {
    this.dispatchPopupEvent("updateSilmMessage", {
      type: "info",
      message: msg
    });
  },
    ...
    this.twitterBackend.showTweet((success, data, status) => {
      if(!success) {
        ...
        const replaseFunc = (data) => {
          ...
          if(text !== "") {
            const replase = this.getDummyTweet();
  ...
  giveMeTweets: function(timelineId = this.currentTimelineId, syncNew = false, cacheOnly = true) {
    const timeline = this.getTimeline(timelineId);
    let callback = (tweets = [], timelineId) => {
  ...
  getDummyTweet: function() {
    ...
    const now = Date.now();
    const id = this.generateDummySnowflake(now);
    return {
      "created_at": utc,
  ...
  generateDummySnowflake: function(now = Date.now()) {
    const table = [
