...
var Renderer = {
  ...
  parseEntities: function(text, entities) {
    ...
    for (i = 0, len = orderedEntities.length; i < len; ++i) {
      ...
      if(entity.type === 'mention') {
        ...
        elements += '<a href="' + entity.url + '" class="handleLink" data-handle-link-base="' + entity.url + '" data-handle-link-expanded="' + entity.expanded_url + '" data-handle-link-media="' + entity.media_url + '"' + title + '>' + entity.display_url + '</a>';
      } else {
  ...
  renderTweet: function (tweet, useColors) {
    ...
    // text_container
    text_container = '<div class="text_container">' + this.parseEntities(text, entities) + '</div>';
