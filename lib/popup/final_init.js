...
function showRateLimit() {
  ...
  if(twitterBackend) {
    ...
    setTimeout(function() {twitterBackend.updateWindowHitsLimit()}, 30 * 1000);
  }
}
...
function handleChromeCommands(command) {
  if(!OptionsBackend.get('use_keyboard_shortcuts')) return;
  switch(command) {
    case "showComposeArea":
...
function initializeWorkspace() {
  if(OptionsBackend.get('use_disable_displayoff')) {
...
function windowOnUnload() {
  ...
  if(OptionsBackend.get('use_keyboard_shortcuts')) {
    chrome.commands.onCommand.removeListener(handleChromeCommands);
  }
  if(OptionsBackend.get('use_disable_displayoff')) {
...
var bindEvents = function() {
  var $window = $(window);
  if(OptionsBackend.get('use_keyboard_shortcuts')) {
    chrome.commands.onCommand.addListener(handleChromeCommands);
    $window.on('keyup', handleKeyboardShortcuts);
  }
  $window.unload(windowOnUnload);

  $(document)
  .on('click', '.msg-trigger-reload', function() {
    chrome.runtime.reload()
  })
