...
class TweetsTimeline {
  ...
  mergeNewTweets() {
    this.tweetsCache = [...this.newTweetsCache, ...this.tweetsCache].sort(TweetsTimeline.sortFromId);
    this.newTweetsCache = [];
  }

  static sortFromId(a, b) {
    const ai = a.id_str;
    const al = ai.length;
    const bi = b.id_str;
    const bl = bi.length;
  ...
  _makeNewTweetsRequestParams() {
    let sinceId = null;
    if(this.tweetsCache.length > 0) {
      for(let tweet of this.tweetsCache) {
        if(!tweet.hasOwnProperty("from") || (tweet.from !== "stream" && tweet.from !== "post")) {
  ...
  _isTweetBlocked(tweet) {
    ...
    if(this.manager.isBlocked(tweet.user.id_str) || (isRT && this.manager.isBlocked(tweet.retweeted_status.user.id_str))) {
      return true;
    }
