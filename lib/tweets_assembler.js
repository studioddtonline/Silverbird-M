...
var Renderer = {
  ...
  entitiesFuncs: {
    ...
    handleHashTag: function(link, value) {
      if(Renderer.isOnPage()) {
        link.attr({'href': TwitterLib.URLS.SEARCH + "%23" + value, target: '_blank'});
        return;
      }
    ...
    handleLink: function(link, baseUrl, expandedUrl, mediaUrl) {
      if(Renderer.isOnPage()) {
        link.attr({'href': baseUrl, target: '_blank'});
        return;
      }
  ...
  parseEntities: function(text, entities) {
    ...
    for (i = 0, len = orderedEntities.length; i < len; ++i) {
      ...
      link = null;
      $($.parseHTML(text.substring(totalInc, indices[0]).replace(/\r|\r?\n/g, '<br>'))).appendTo(elements);
      if (entity.type === 'mention') {
        ...
        $($.parseHTML(text.substring(indices[0], indices[1]).replace(/\r|\r?\n/g, '<br>'))).appendTo(elements);
      }
      if (link) {
    ...
    $($.parseHTML(text.substring(totalInc, text.length).replace(/\r|\r?\n/g, '<br>'))).appendTo(elements);
    return elements;
  },
  ...
  renderTweet: function (tweet, useColors) {
    ...
    var compliantTDR, nameAttribute;
    ...
    var statusLinkSpan = $($.parseHTML('<span class="timestamp"><a title="' + Renderer.getTimestampAltText(tweet.created_at) + '" href="' + statusLinkDst + '">' + Renderer.getTimestampText(tweet.created_at) + '</a></span>'));
    if(Renderer.isOnPage()) {
    ...
    // tweet space
    var overlayStyle = '', newActions_container = '';
    if(this.isComplete()) {
    ...
    var tweetSpace = $($.parseHTML('<div class="tweet_space"><div class="chromed_bird_tweet tweet" timelineid="' + tweetTimeline + '" tweetid="' + tweet.id + '"><div class="tweet_overlay" style="' + overlayStyle + '"><div class="first_container"><div class="profile_container"></div><div class="header_container"></div><div class="text_container"></div><div class="footer_container"></div></div>' + newActions_container + '</div></div></div>'));

    // profile_container
    if(tweet.retweeted_status) {
      tweetSpace.find('.profile_container').html($.parseHTML('<img class="profile retweet_source" src="' + user.profile_image_url + '"/><img class="profile retweet_retweeter" src="' + tweet.user.profile_image_url + '"/>'));
      Renderer.createUserActionMenu(tweetSpace.find('.retweet_source'), user.screen_name);
      Renderer.createUserActionMenu(tweetSpace.find('.retweet_retweeter'), tweet.user.screen_name);
    } else {
      tweetSpace.find('.profile_container').html($.parseHTML('<img class="profile" src="' + user.profile_image_url + '" />'));
      Renderer.createUserActionMenu(tweetSpace.find('.profile'), user.screen_name);
    }

    // header_container
    var userName = '', userVerified = '', userProtected = '';
    if(user.verified) {
    ...
    if(nameAttribute == "both") {
      userName = '<div class="primary_name"><a href="#" class="user" screen_name="' + user.screen_name + '">' + user.name + '</a>' + userVerified + userProtected + '</div><div class="secondary_name"><a href="#" class="user" screen_name="' + user.screen_name + '">@' + user.screen_name + '</a></div>';
    } else if(nameAttribute == "screen_name") {
      userName = '<div class="primary_name"><a href="#" class="user" screen_name="' + user.screen_name + '" title="' + user.name + '">@' + user.screen_name + '</a>' + userVerified + userProtected + '</div>';
    } else if(nameAttribute == "name") {
      userName = '<div class="primary_name"><a href="#" class="user" screen_name="' + user.screen_name + '" title="@' + user.screen_name + '">' + user.name + '</a>' + userVerified + userProtected + '</div>';
    }
    ...
    Renderer.createUserActionMenu(tweetSpace.find('.primary_name').find('a'), user.screen_name);
    if(nameAttribute == "both") {
      Renderer.createUserActionMenu(tweetSpace.find('.secondary_name').find('a'), user.screen_name);
    }
    ...
    if(this.isComplete()) {
      ...
      if(tweet.retweeted_status) {
        if(tweet.user.screen_name == tweetManager.twitterBackend.username()) {
          ...
          footerContent += '<span class="inRetweet">' + chrome.i18n.getMessage("retweetedBy_prefix") + '<a href="' + TwitterLib.URLS.BASE + tweet.user.screen_name + '">' + tweet.user.screen_name + '</a>' + chrome.i18n.getMessage("retweetedBy_suffix") + '</span>';
        }
      ...
      if(templateId == TimelineTemplate.SENT_DMS) {
        footerContent += '<span class="dm_recipient">' + chrome.i18n.getMessage("sentTo_prefix") + '<a href="#">' + tweet.recipient.name + '</a>' + chrome.i18n.getMessage("sentTo_suffix") + '</span>';
      }
      // geo
      if(tweet.geo) {
      ...
      if(tweet.retweeted_status && tweet.user.screen_name != tweetManager.twitterBackend.username()) {
        Renderer.createUserActionMenu(tweetSpace.find('.inRetweet').find('a'), tweet.user.screen_name);
      }
      if(tweet.source) {
      ...
      if(templateId == TimelineTemplate.SENT_DMS) {
        Renderer.createUserActionMenu(tweetSpace.find('.dm_recipient').find('a'), tweet.recipient.screen_name);
      }
      if(tweet.geo) {
