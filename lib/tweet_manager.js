...
function TweetManager() {
  ...
  this.displayStreamingCallback = function() {};
  this.trendsCallback = function() {};
  this.savedSearchCallback = function() {};
  ...
  this.warningsCallback = function() {};
  ...
  this.twitterBackend = new TwitterLib(
    (function(self) {
      return function() {
        ...
        if(!self.previousUserId.val() || self.previousUserId.val() === this.userid()) {
        ...
        $.when(
          ...
          self.retrieveMutesUsers(null, -1),
...
TweetManager.prototype = {
  ...
  retrieveBlockedUsers: function(D, cursor) {
    ...
    this.twitterBackend.blockedUsers((function(self, deferred) {
      return function(success, data, status, context) {
        ...
        if(context && data.next_cursor > 0 && data.next_cursor > context) {
          self.retrieveBlockedUsers(deferred, data.next_cursor);
          return deferred.notify();
        } else {
          self.workerPurgeRemovedUsers();
  ...
  workerPurgeRemovedUsers: function() {
    if(!this.worker) this.worker = new Worker('./lib/worker/purgeBlockedTweets.js');
    this.eachTimeline((function(self) {
      return function(timeline) {
        self.worker.addEventListener('message', function(event) {
          timeline.timelineCache = event.data;
          self.worker.removeEventListener('message', arguments.callee);
        });
        self.worker.postMessage({
          tweetsCache: timeline.tweetsCache,
          blockedUserNames: self.blockedUserNames
        });
      };
    })(this), true);
  },
  ...
  getListId: function(timelineId) {
    var timeline = this.timelines[timelineId || this.currentTimelineId];
    if(!timeline) return null;
  ...
  onStreamData: function(data) {
    ...
    if(data.disconnect || data.event === StreamListener.events.DISCONNECTED) {
      this.displayStreamingCallback(false);
      return;
    }
    if(data.event === StreamListener.events.CONNECTED) {
      this.displayStreamingCallback(true);
      return;
    }
    if(data.warning) {
    ...
    if(data.event) {
      ...
      switch(data.event) {
        ...
        case 'block':
          ...
          return; // do not notification
        case 'unblock':
          delete this.blockedUserNames[data.target.screen_name];
  ...
  _removeUser: function(context) {
    if(context.unfollow) {
      delete this.followingUsersMap[context.userData.screen_name];
    ...
    if(context.muting) {
      this.mutingUsersMap[context.userData.screen_name] = context.userData;
    }
    this.resetTimeline(TimelineTemplate.HOME);
    this.resetTimeline(TimelineTemplate.UNIFIED);

    if(context.blocked) {
      this.blockedUserNames[context.userData.screen_name] = context.userData;
      this.workerPurgeRemovedUsers();
    }
  },
  ...
  setAlarms: function() {
    ...
    chrome.alarms.create('retrieve_blocked_users', {
      delayInMinutes: retrieve_blocked_users_interval,
    ...
    chrome.alarms.create('retrieve_muting_users', {
      delayInMinutes: retrieve_muting_users_interval,
  ...
  _addFollowingUser: function(userData) {
    this.followingUsersMap[userData.screen_name] = userData;
  ...
  registerPopupCallbacks: function(callbacks) {
    ...
    this.warningsCallback = callbacks.warnings || this.warningsCallback || nobehavior;
    this.newTweetsCallback = callbacks.newTweets || this.newTweetsCallback || nobehavior;
    this.displayStreamingCallback = callbacks.displayStreaming || this.displayStreamingCallback || nobehavior;
    this.trendsCallback = callbacks.trends || this.trendsCallback || nobehavior;
    this.savedSearchCallback = callbacks.savedSearch || this.savedSearchCallback || nobehavior;
    if(this.warningMessage && this.warningsCallback) {
      this.warningsCallback(this.warningMessage, this.warningMessageHTML);
    }
    this.displayStreamingCallback(StreamListener.getStatus());
    this.trendsCallback();
    this.savedSearchCallback();
    callbacks = null;
  },

  unregisterPopupCallbacks: function() {
    var nobehavior = function() {};
    this.warningsCallback = nobehavior;
    this.newTweetsCallback = nobehavior;
    this.displayStreamingCallback = nobehavior;
  },
    ...
    this.twitterBackend.mutesUsers((function(self, deferred) {
      return function(success, data, status, context) {
        ...
        for(var i = 0, len = mutesUsers.length; i < len; ++i) {
          ...
          self.mutingUsersMap[user.screen_name] = user;
        }
        if(context && data.next_cursor > 0 && data.next_cursor > context) {
          self.retrieveMutesUsers(deferred, data.next_cursor);
          return deferred.notify();
        } else {
          return deferred.resolve();
        }
  ...
  unmuteUser: function(callback, username) {
    this.twitterBackend.destroyMutes((function(self) {
      return function(success, userData) {
        if(success) {
          delete self.mutingUsersMap[userData.screen_name];
  ...
  getMutingUsersMap: function() {
    return this.mutingUsersMap;
  }
};
