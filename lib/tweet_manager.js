...
TweetManager.prototype = {
  ...
  */
  ...
  onHitsUpdated: function(rateLimits) {
    ...
    var nextResetDate = currentDate;
    for(var key in rateLimits) {
      if(!rateLimits.hasOwnProperty(key)) continue;
      var value = rateLimits[key];
      if(value.remaining < 2) {
        apihits = true;
        var newResetDate = parseInt(value.reset, 10);
        if(newResetDate > nextResetDate) nextResetDate = newResetDate;
      }
    }
    if(apihits) {
      var resetDateObj = (new Date(nextResetDate * 1000));
      this.autoClearWarning = true;
      this.setWarning(
        chrome.i18n.getMessage(
          "exceededAPIHits",
          [chrome.extension.getURL('options.html'), resetDateObj.toLocaleDateString(), resetDateObj.toLocaleTimeString()]
        ),
        true
      );
      return;
