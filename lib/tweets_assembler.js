...
var Renderer = {
  ...
  renderTweet: function (tweet, useColors, nameAttribute) {
    ...
    if(!this.isNotification()) {
      var cloneBaseImg = $(document.createElement("img")),
          cloneBaseAnchor = $(document.createElement('a')).attr('href', '#'),
          cloneBaseBr = $(document.createElement("br"));
      ...
      if(templateId != TimelineTemplate.RECEIVED_DMS && templateId != TimelineTemplate.SENT_DMS) {
        if(tweet.favorited) {
          cloneBaseImg.clone()
            .addClass('starred').attr('title', chrome.i18n.getMessage('unmarkFavorite')).attr('src', 'img/star_hover.png')
            .click(function() {
              Composer.unFavorite(this.parentNode.parentNode.parentNode);
            }).appendTo(actions);
        } else {
          cloneBaseImg.clone()
            .addClass('unstarred').attr('title', chrome.i18n.getMessage('markFavorite')).attr('src', 'img/star.png')
            .click(function() {
