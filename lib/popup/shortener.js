var Shortener = {
  ...
  shortenCurrentPage: function(withoutQuery) {
    chrome.tabs.query(Shortener.CHROME_QUERY_PARAMS, function(tabs) {
      tabs = tabs.filter(function(entry) {
        if(entry.url && /^https?:\/\//i.test(entry.url)) {
          return true;
        }
      });
      if(tabs.length > 0) {
        var url = tabs[0].url, title = tabs[0].title;  //TODO: should be selectable
        $('#shortener_text').val(url);
        Shortener.shortenIt({title: title, withoutQuery: withoutQuery});
      } else {
