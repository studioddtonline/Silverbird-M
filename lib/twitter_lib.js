...
TwitterLib.prototype = {
  ...
  signOauth: function(xhr, url, params, method, useBearer) {
    if(useBearer
    && this.bearerTokenData.val()) {
  ...
  ajaxRequest: function(url, callback, context, requestParams, httpMethod, overriddenTimeout, useBearer) {
    if(!httpMethod) {
      httpMethod = "GET";
    }
    if(!requestParams) {
    ...
    var beforeSendCallback = function(request, settings) {
      ...
      _this.signOauth(request, signingUrl, requestParams, httpMethod, useBearer);
    };
    ...
    .always(function(data, status, request){
      try {
        ...
        for(var i = 0, j = allHeaders.length; i < j; i++) {
          if((/X-Rate-?Limit-Remaining/i).test(allHeaders[i])) {
            rateLimits.remaining = parseInt(allHeaders[i].split(/:\s*/)[1], 10);
          } else if((/X-Rate-?Limit-Reset/i).test(allHeaders[i])) {
            rateLimits.reset = parseInt(allHeaders[i].split(/:\s*/)[1], 10) * 1000;
          } else if((/X-Rate-?Limit-Limit/i).test(allHeaders[i])) {
            rateLimits.limit = parseInt(allHeaders[i].split(/:\s*/)[1], 10);
          } else if((/X-Access-Level/i).test(allHeaders[i])) {
            _this.lastAccessLevel = allHeaders[i].split(/:\s*/)[1];
          }
        }
      ...
      if(success) {
        $.each(data.resources, function(apiFamilies, api) {
          $.each(api, function(apiEndpoint, apiRateInfo) {
            ...
            if(!_this.rateLimits[apiName]) _this.rateLimits[apiName] = {};
            var rateLimits = _this.rateLimits[apiName];
  ...
  searchTimeline: function(callback, context, params) {
    ...
    params.include_entities = 'true';
    this.ajaxRequest('search/tweets', callback, context, params, "GET", null, OptionsBackend.get('use_bearer_token'));
  },
  ...
  usersTimeline: function(callback, context, params) {
    params.include_rts = 'true';
    this.ajaxRequest('statuses/user_timeline', callback, context, params, "GET", null, OptionsBackend.get('use_bearer_token'));
  },
  ...
  retrieveBearerToken: function(callback) {
    ...
    .done(function(data, status, request) {
      _this.bearerTokenData.save(data.access_token);
      callback(true, data.access_token, status, null, request);
    })
      ...
      try {
        if(request && request.readyState == 4) {
          ...
          if(request.status == 429) {
            ...
            fmtError = status + '(' + res.errors.message + ')';
          }
        }
      } catch(e) {
        fmtError = 'Application-only authentication Error (' + error + ')';
      }
      callback(false, null, fmtError, null, request);
    });
  }
};
