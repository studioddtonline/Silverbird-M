  ...
  '_scraping': {
    value: function(result) {
      ...
      try {
        ...
        if(doc) {
          var image = doc.querySelector('.animated-gif-thumbnail') // Twitter/vine
          ...
          if(thumb) {
            ...
            if(this._regexp.noScheme.test(thumb)) {
              thumb = thumb.replace(this._regexp.noScheme, this._a.protocol + '//');
            } else if(this._regexp.noHostname.test(thumb)) {
              thumb = thumb.replace(this._regexp.noHostname, this._a.origin + '/');
