...
function onTimelineRetrieved(tweets, timelineId) {
  ...
  if(tweets) {
    ...
    if(baseErrorMsg === '(This user is protected)') {
      ...
      var errorMsg = "", tryAgainFunc = null;
      if(baseErrorMsg === 504) {
        ...
        tryAgainFunc = loadTimeline;
      }
      Renderer.showError(errorMsg, tryAgainFunc);
    }
  }
...
function displayStreamingStatus() {
  var state = backgroundPage.StreamListener.getStatus();
  if(state == 'disallow') {
    ...
    switch(change.name) {
      case 'cachedTrendingTopics':
        ...
        && OptionsBackend.get('notify_update_trends')) {
          var oldTrends = (change.oldValue.trends || []).slice(0, 10);
          tweetManager.createUpdateTrendsNotification(newValue.trends.slice(0, 10).filter(function(entity) {
...
function enableTweetManagerObserver() {
  if(!tweetManager.isObserved) {
    tweetManager.isObserved = true;
    Object.observe(tweetManager, tweetManagerObserver);
    console.log('Enable: Observer');
  }
}

function disableTweetManagerObserver() {
  if(tweetManager.isObserved) {
    Object.unobserve(tweetManager, tweetManagerObserver);
    tweetManager.isObserved = false;
...
function bindEvents() {
  ...
  $(window)
  .on({
    'focus.popup': function() {
      enableTweetManagerObserver();
    },
    'blur.popup': function() {
      disableTweetManagerObserver();
    },
    'beforeunload.popup': function() {
      disableTweetManagerObserver();
    },
    'updateTrends': function(event) {
      console.log(event);
    },
    'updateLists': function(event) {
      Lists.update();
    },
    'updateSavedSearch': function(event) {
      loadSavedSearch();
    },
    ...
    'updateWarningMessage': function(event) {
      ...
      if(event.detail && event.detail !== "") {
        Renderer.warningsCallback(event.detail, false);
      }
    }
  });
