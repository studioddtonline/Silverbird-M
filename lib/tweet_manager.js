  ...
  this.twitterBackend = new TwitterLib(
    (function(self) {
      return function(success, data) {
        ...
        if(!self.previousUserId.val() || self.previousUserId.val() === data.id_str) {
...
TweetManager.prototype = {
      ...
      switch(data.event) {
        ...
        case 'favorite':
          if(data.source.id_str == this.twitterBackend.userid()) {
            ...
            if(data.target_object) {
              var favoriteUrl = `https://twitter.com/${data.target.screen_name}/status/${data.target_object.id_str}`;
              var favoriteDisplayUrl = `${favoriteUrl.substr(8, 26)}${String.fromCharCode(8230)}`;
              context.text += ` ${favoriteDisplayUrl}`;
              var index = context.text.indexOf(favoriteDisplayUrl);
              var favoriteEntitiy = {
                url: favoriteUrl,
                display_url: favoriteDisplayUrl,
                expanded_url: favoriteUrl,
                indices: [index, (index + favoriteDisplayUrl.length)]
              };
              context.entities.urls.push(favoriteEntitiy);
        ...
        case 'unfavorite':
          if(data.source.id_str == this.twitterBackend.userid()) {
            ...
            context.entities.user_mentions.push(fixingEntities(context.text, data.source.screen_name, entityForUserMentions));
            if(data.target_object) {
              var unFavoriteUrl = `https://twitter.com/${data.target.screen_name}/status/${data.target_object.id_str}`;
              var unFavoriteDisplayUrl = `${unFavoriteUrl.substr(8, 26)}${String.fromCharCode(8230)}`;
              context.text += ` ${unFavoriteDisplayUrl}`;
              var index = context.text.indexOf(unFavoriteDisplayUrl);
              var unFavoriteEntitiy = {
                url: unFavoriteUrl,
                display_url: unFavoriteDisplayUrl,
                expanded_url: unFavoriteUrl,
                indices: [index, (index + unFavoriteDisplayUrl.length)]
              };
              context.entities.urls.push(unFavoriteEntitiy);
              context.quoted_status = data.target_object;
              context.quoted_status_id_str = data.target_object.id_str;
            }
          }
    ...
    this.twitterBackend.retrieveConfiguration((function(self) {
      return function(success, data) {
        if(success && data) {
          ...
          // current values
          self.twitterConfiguration.characters_reserved_per_media = 23;
          self.twitterConfiguration.max_media_per_upload = 1;
          self.twitterConfiguration.photo_size_limit = 3145728;
          self.twitterConfiguration.short_url_length = 22;
          self.twitterConfiguration.short_url_length_https = 23;
          self.dm_text_character_limit = 140;
        }
      };
    })(this));
  },
  ...
  createUpdateTrendsNotification: function(trends) {
    if(!trends || !Array.isArray(trends) || trends.length === 0) {
      return;
    }
    var context = this.getDummyTweet();
    var codePointLength = function(str) {
      'use strict';
      if(!str) {
        throw new TypeError('missing strings');
      }
      let length = 0;
      for(let s of str) {
        length++;
    ...
    trends.forEach(function(trend) {
      if(trend.name.substr(0, 1) === '#') {
        var index = codePointLength(context.text);
        var length = codePointLength(trend.name);
        context.text += trend.name + "\n";
        context.entities.hashtags.push({
          text: trend.name.substr(1),
          indices: [index, index + length]
        });
      } else {
        var display_url = decodeURIComponent((trend.query).replace('+', '%20'));
        var expanded_url = trend.url;
        var index = codePointLength(context.text);
        var length = codePointLength(display_url);
        context.text += display_url + "\n";
