...
$.extend(UnifiedTweetsTimeline.prototype, TweetsTimeline.prototype, {
  ...
  init: function() {
    ...
    TimelineTemplate.eachTimelineTemplate(closure_function(this), true);
  },
  ...
  /* overridden */
  giveMeTweets: function(callback, syncNew, cacheOnly, keepCache) {
    ...
    if(cacheOnly || syncNew) {
      if(this.currentScroll === 0 && !keepCache) {
        ...
        */
        while(resultTweetsList.length < requiredTweets) {
          ...
          if(isLastInTimelineCache) {
            ...
            */
            if(self.lastRequestedId[nextTweet.timelineId] == nextTweet.id) {
            ...
            var newJoinableCallback = Utils.makeJoinableCallback(1, eachFunction(self), joinFunction(self));
            var wantedTimeline = self.timelines[nextTweet.timelineId];
            ...
            wantedTimeline.giveMeTweets(function(returnedTweets) {
              if(returnedTweets) { // Success
                self.lastRequestedId[nextTweet.timelineId] = nextTweet.id;
              }
    ...
    */
    ...
    var joinableCallback = Utils.makeJoinableCallback(joinCount, eachFunction(this), joinFunction(this));
