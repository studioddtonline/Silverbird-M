  ...
  renderTweet: function (tweet, useColors) {
    ...
    if(!hiddenUserIcons) {
      if(tweet.retweeted_status) {
        ...
        Renderer.createUserActionMenu(tweetSpace.find('.retweet_source')[0], user.screen_name);
        Renderer.createUserActionMenu(tweetSpace.find('.retweet_retweeter')[0], tweet.user.screen_name);
      } else {
        tweetSpace.find('.profile_container').html($.parseHTML('<img class="profile" src="' + user.profile_image_url + '" />'));
        Renderer.createUserActionMenu(tweetSpace.find('.profile')[0], user.screen_name);
      }
    }
    ...
    Renderer.createUserActionMenu(tweetSpace.find('.primary_name').find('a')[0], user.screen_name);
    if(nameAttribute == "both" && !displaySimpleName) {
      Renderer.createUserActionMenu(tweetSpace.find('.secondary_name').find('a')[0], user.screen_name);
    }
    ...
    if(this.isComplete() && !hiddenFooter) {
      ...
      if(tweet.retweeted_status && tweet.user.screen_name != tweetManager.twitterBackend.username()) {
        Renderer.createUserActionMenu(tweetSpace.find('.inRetweet').find('a')[0], tweet.user.screen_name);
      }
      if(tweet.source) {
      ...
      if(templateId == TimelineTemplate.SENT_DMS) {
        Renderer.createUserActionMenu(tweetSpace.find('.dm_recipient').find('a')[0], tweet.recipient.screen_name);
      }
      if(tweet.geo) {
