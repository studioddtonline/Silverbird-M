...
TwitterLib.prototype = {
  ...
  ajaxRequest: function(url, callback, context, requestParams, httpMethod, overriddenTimeout) {
    ...
    var beforeSendCallback = function(request, settings) {
      ...
      if(_this.rateLimits[apiName].remaining == 0) {
        var resetRemain = now - (_this.rateLimits[apiName].reset * 1000);
        if(resetRemain < 0) {
          request.abort();
          return;
        }
        ...
        $.each(allHeaders, function(index, header) {
          switch( true ) {
            ...
            case (/X-Rate-?Limit-Reset/i).test(header):
              _this.rateLimits[apiName].reset = parseInt(header.split(/:\s*/)[1], 10);
              break;
  ...
  updateWindowHitsLimit: function() {
    ...
    this.ajaxRequest("application/rate_limit_status", function(success, data, status, context, xhr) {
      if(success) {+        $.each(data.resources, function(apiFamilies, api) {
          $.each(api, function(apiEndpoint, apiRateInfo) {
            var apiName = apiEndpoint.split('/').slice(1, 3).join('/');
            _this.rateLimits[apiName] = apiRateInfo;
            _this.rateLimits[apiName].remaining = parseInt(_this.rateLimits[apiName].remaining, 10);
            _this.rateLimits[apiName].reset = parseInt(_this.rateLimits[apiName].reset, 10);
            _this.rateLimits[apiName].limit = parseInt(_this.rateLimits[apiName].limit, 10);
          });
        });
      }
      if(xhr) {
