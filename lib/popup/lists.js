  ...
  init: function(force) {
    ...
    tweetManager.lists(force, function(success, lists, status) {
      ...
      tweetManager.eachTimeline(function(timeline) {
        ...
        var baseOpEl = $(document.createElement("option")), df = $(document.createDocumentFragment());
        for(var j = 0; j < lists.length; ++j) {
          df.append(baseOpEl.clone().val(lists[j].uri).text(lists[j].name));
        }
        $listSelect
          .empty()
          .append(baseOpEl.clone().val(_this.selector_value).text(chrome.i18n.getMessage("selectList")))
          .append(df)
          .append(baseOpEl.clone().val(_this.update_value).text(chrome.i18n.getMessage("updateLists")));
