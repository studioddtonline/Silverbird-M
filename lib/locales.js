...
LocaleTable.prototype = {
    ...
    var closure_function = function(self) {
      return function(locales) {
        ...
        if(self.waitingCallback) {
          self.waitingCallback(locales);
          self.waitingCallback = null;
        }
      };
    };
    this.defaultLocale = chrome.runtime.getManifest().default_locale;
    this.loadLocaleMessages(closure_function(this));
  },
  ...
  loadLocaleMessages: function(callback) {
    ...
    var locales = {}, closure_function = function(self) {
      return function(localeCode, data) {
        delete remainingLocalesMap[localeCode];
        if(data) {
          locales[localeCode] = data;
          self.parsePlaceholders(data);
        }
        if(Util.isEmptyObject(remainingLocalesMap)) {
          callback(locales);
        }
      };
    }, updateLocaleCallback = closure_function(this);
