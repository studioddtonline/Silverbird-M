var Renderer = {
  ...
  parseUrlsEntity: function(texts = [], entity = [], tweetSpaceId = "", quotedStatus = {id_str: null}) {
    ...
    entity.forEach((item) => {
      if(!!item.indices || Array.isArray(item.indices)) {
        const [indexStart, indexEnd] = item.indices;
        const decodedUrl = decodeURIComponent(item.expanded_url);
        const spliceLength = indexEnd - indexStart;
        const spliceArray = " ".repeat(spliceLength - 1).split(" ");
        if(searchRegexp.test(decodedUrl)) {
          ...
          spliceArray[0] = `<span><span class="glyphicon glyphicon-link"></span><a href="${item.url}" class="expandInReply" data-handle-link-base="${item.url}" data-handle-link-expanded="${item.expanded_url}" data-handle-link-media="${item.media_url_https || ""}" data-expand-in-reply-tweet="${escape(JSON.stringify(quotedTweetEntity))}" data-expand-in-reply-id="${tweetSpaceId}" title="${chrome.i18n.getMessage("expand_quote_tweet")}">${item.display_url}</a></span>`;
        } else {
          spliceArray[0] = `<a href="${item.url}" class="handleLink" data-handle-link-base="${item.url}" data-handle-link-expanded="${item.expanded_url}" data-handle-link-media="${item.media_url_https || ""}" title="${item.expanded_url}">${item.display_url}</a>`;
        }
  ...
  renderTweet: function (tweet, now, displayOptions) {
    ...
    if(!displayOptions.hiddenRetweetInfo && (!!tweet.retweeted_status || !!tweet.current_user_retweet)) {
      ...
      if(selfTweet || !!tweet.current_user_retweet) {
        retweets_container += `<span class="selfRetweet">${chrome.i18n.getMessage("retweetedByMe")}</span>`;
      } else {
        retweets_container += `<span class="inRetweet">${Renderer.buildIcon(tweet, false, {hiddenUserIcons: displayOptions.hiddenUserIcons, iconSize: "retweeter"})}${chrome.i18n.getMessage("retweetedBy_prefix")}<a href="${TwitterLib.URLS.BASE}${tweet.user.screen_name}" data-user-id="${tweet.user.id_str}" data-user-name="${tweet.user.screen_name}" class="createUserActionMenu">${tweet.user.screen_name}</a>${chrome.i18n.getMessage("retweetedBy_suffix")}</span>`;
      }
  ...
  buildName: function(tweet, isRetweet = true, displayOptions = {displaySimpleName: false, nameAttribute: "both"}) {
    ...
    const userVerified = (!!user.verified && !displayOptions.displaySimpleName)? `<span class="glyphicon glyphicon-check verifiedAccount" title="${chrome.i18n.getMessage('verified_account')}"></span>`: "";
    const userProtected = (!!user.protected && !displayOptions.displaySimpleName)? `<span class="glyphicon glyphicon-lock protectedAccount" title="${chrome.i18n.getMessage('protected_account')}"></span>`: "";
  ...
  buildFromApp: function(tweet, isRetweet = true, displayOptions = {hiddenClientName: false}) {
    ...
    if(!displayOptions.hiddenClientName && !!source) {
      ...
      ret = `<span class="from_app">${chrome.i18n.getMessage("fromApp_prefix")}${source}${chrome.i18n.getMessage("fromApp_suffix")}</span>`;
    }
    return ret;
  },
  ...
  buildReply: function(tweet, isRetweet = true, displayOptions = {hiddenReplyInfo: false}) {
    ...
    if(!displayOptions.hiddenReplyInfo && !!baseTweet.in_reply_to_status_id) {
      ret = `<span class="inReply">${chrome.i18n.getMessage("inReply_prefix")}<a class="expandInReply" data-expand-in-reply-tweet="${escape(JSON.stringify({"in_reply_to_status_id": baseTweet.in_reply_to_status_id}))}" data-expand-in-reply-id="${tweet.silm_tweetSpaceId}" href="#">${baseTweet.in_reply_to_screen_name}</a>${chrome.i18n.getMessage("inReply_suffix")}</span>`;
    }
    return ret;
  },
  ...
  buildFavoriteAction: function(tweet, isRetweet = true, isDM = false) {
    ...
    if(!isDM) {
      ...
      if(!!baseTweet.favorited) {
        ret += `glyphicon-heart action_unlike" title="${chrome.i18n.getMessage('unmarkLike')}"`;
      } else {
        ret += `glyphicon-heart-empty action_like" title="${chrome.i18n.getMessage('markLike')}"`;
      }
  ...
  buildReplyAction: function(tweet, isRetweet = true, isDM = false, isSelf = false) {
    ...
    if(!(isDM && isSelf)) {
      const baseTweet = (isRetweet && !!tweet.retweeted_status)? tweet.retweeted_status: tweet;
      ret = `<span class="glyphicon glyphicon-reply new_actions_item action_reply" title="${chrome.i18n.getMessage("Reply")}" data-reply-target-id="${baseTweet.id_str}" data-reply-target-name="${baseTweet.user.screen_name}" data-reply-to-dm="${isDM}"></span>`;
    }
    return ret;
  },
  ...
  buildRetweetAction: function(tweet, isRetweet = true, isDM = false, isSelf = false) {
    ...
    if(!isDM && !(baseTweet.user.protected && !isSelf) && !baseTweet.retweeted) {
      ret += `<span class="glyphicon glyphicon-retweet new_actions_item action_retweet" title="${chrome.i18n.getMessage("Retweet")}" data-retweet-target-id="${baseTweet.id_str}"></span>`;
    }
    if(!isDM && !(baseTweet.user.protected && !isSelf)) {
      ret += `<span class="glyphicon glyphicon-comment new_actions_item action_quote" title="${chrome.i18n.getMessage("quoteTweet")}" data-quote-tweet-url="${Renderer.buildTimestampUrl(tweet, isRetweet)}"></span>`
    }
    return ret;
  },
  ...
  buildDeleteAction: function(tweet, isDM = false, isSelf = false) {
    let ret = "";
    if(!!isDM || !!isSelf) {
      ret = `<span class="glyphicon glyphicon-trash new_actions_item action_delete_tweet" title="${chrome.i18n.getMessage("Delete")}" data-delete-target-id="${tweet.id_str}" data-timeline-id="${tweet.silm_tweetTimelineId}"></span>`;
    }
    return ret;
  },
  ...
  buildDeleteRetweetAction: function(tweet, isSelf = true) {
    ...
    if((!!tweet.retweeted_status && !!isSelf) || !!tweet.current_user_retweet) {
      ...
      ret = `<span class="glyphicon glyphicon-remove-circle new_actions_item action_cancel_retweet" title="${chrome.i18n.getMessage("deleteRT")}" data-delete-target-id="${tweetId}" data-timeline-id="${tweet.silm_tweetTimelineId}"></span>`;
    } 
    return ret;
  },
  ...
  buildDmAction: function(tweet, isRetweet = true, isDM = false) {
    ...
    if(!isDM && enableDM) {
      ret = `<span class="glyphicon glyphicon-envelope new_actions_item action_message" title="${chrome.i18n.getMessage("directMessage")}" data-message-target-name="${baseTweet.user.screen_name}"></span>`;
    }
    return ret;
  },
