...
var Renderer = {
  ...
  parseEntities: function(text, entities, extended_entities) {
    ...
    for (i = 0, len = orderedEntities.length; i < len; ++i) {
      ...
      } else if(entity.type === "media" && extendedMediaEntities.length > 0) {
        elements += extendedMediaEntities.map(function(value) {
          if(value.display_url[value.display_url.length - 1].charCodeAt(0) == 8230) { // Ends with ...
            title = ' title="' + value.expanded_url + '"';
          }
          return '<a href="' + value.url + '" class="handleLink" data-handle-link-base="' + value.url + '" data-handle-link-expanded="' + value.expanded_url + '" data-handle-link-media="' + value.media_url_https + '"' + title + '>' + value.display_url + '</a>';
        }).join(' ');
