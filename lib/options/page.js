...
$(function() {
  ...
  ['unified', 'home', 'mentions', 'dms', 'lists', 'search', 'favorites', 'notification'].forEach(function(timeline) {
    ...
    } else if(timeline === 'notification') {
      inputVisible = $('input[name="notification_include_unified"]');
      handlerCheck = function() {
        if(inputVisible.is(':checked')) {
          $('input[name="notify_update_trends"]').removeAttr('disabled');
        } else {
  ...
  options.onsaveChangedOption(function(optionName, oldValue, newValue) {
    ...
    if((idx = optionName.indexOf('_visible')) != -1) {
      ...
      OptionsBackend.setDefault('shortener_refresh_token');
