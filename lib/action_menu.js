  ...
  $.actionMenu.prototype = {
    close: function() {
      if(this.visible && !this.closing) {
        this.closing = true;
        this.actionMenuEl.stop(true, true).effect('puff', {}, 150, () => {
          this.actionMenuEl.css({display: 'none', position: 'absolute'});
          this.visible = false;
          this.closing = false;
        });
      }
    ...
    invertActions: function(direction, lastDirection) {
      const actionArea = this.actionMenuEl.find(".action_area");
