var Composer = {
  ...
  init: function() {
    if(tweetManager.composerData.isComposing) {
      Composer.initMessage(tweetManager.composerData.saveMessage, tweetManager.composerData.replyId, tweetManager.composerData.replyUser, tweetManager.composerData.mediaIds, false);
    }
    Composer.textareaChanged();
    Composer.fileReader.addEventListener('load', Composer.fileReaderOnload);
  },

  initMessage: function(message, replyId, replyUser, mediaIds, shouldAnimate) {
    ...
    if(str == '') {
      ...
      for(var mediaId of Composer.mediaIds.keys()) {
        if(Composer.mediaIds.size > Composer.maxAttachableMedia) {
          ...
          var shiftedPreview = $('#thumbnail-' + mediaId);
          if(shiftedPreview.length > 0) {
            shiftedPreview.off('.thumbnail').remove();
          }
        } else break;
      }
    }
  ...
  sendTweet: function () {
    ...
    if(regexpResult !== null) {
      ...
      tweetManager.enqueueTweet(message, regexpResult[1], Composer.replyUser, true, arrayedMediaIds);
    } else {
      tweetManager.enqueueTweet(message, Composer.replyId, Composer.replyUser, false, arrayedMediaIds);
    }
