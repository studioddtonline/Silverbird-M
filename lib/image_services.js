...
ImageService.addService('instagr.am', {
  thumb: function(path, url) { return url + (url[url.length - 1] == '/' ? '' : '/') + 'media?size=t' }
});
...
ImageService.addService('lockerz.com', {
  thumb: 'http://api.plixi.com/api/tpapi.svc/imagefromurl?url=$2&size=thumbnail'
});
...
ImageService.addService('p.twipple.jp', {
  thumb: 'http://p.twpl.jp/show/thumb/$1'
});
