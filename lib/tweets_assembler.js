...
var Renderer = {
  ...
  parseEntities: function(text, entities) {
    ...
    var totalInc = 0, elements = '', i, len, entity, indices, link;
    for (i = 0, len = orderedEntities.length; i < len; ++i) {
      ...
      if (entity.type === 'mention') {
        elements += '@<a href="#" class="createUserActionMenu" data-create-user-action-menu="' + entity.screen_name + '">' + entity.screen_name + '</a>';
        ...
        elements += '<a href="' + entity.url + '" ' + title + 'class="handleLink" data-handle-link-base="' + entity.url + '" data-handle-link-expanded="' + entity.expanded_url + '" data-handle-link-media="' + entity.media_url + '">' + entity.display_url + '</a>';
      } else {
  ...
  renderTweet: function (tweet, useColors) {
    ...
    if(tweet.retweeted_status) {
      ...
      if(tweet.user.screen_name == tweetManager.twitterBackend.username() && !tweetManager.isRetweet(tweet)) {
    ...
    // tweet space
    var timestamp_content, timestamp_url = '', timestamp_target = '',
        timestamp_topRight = '', timestamp_bottomLeft = '',
        overlayStyle = '', profile_container = '', header_container = '',
    ...
    if(!hiddenTimestamp) {
      ...
      if(Renderer.isOnPage()) {
        timestamp_target = 'target="_blank" ';
      } else {
        timestamp_target = 'class="handleLink" data-handle-link-base="' + timestamp_url + '" ';
      }
      timestamp_content = '<span class="timestamp"><a ' + timestamp_target + 'title="' + Renderer.getTimestampAltText(tweet.created_at) + '" href="' + timestamp_url + '">' + Renderer.getTimestampText(tweet.created_at) + '</a></span>';
    }
    ...
    if(nameAttribute == "both") {
      header_container = '<div class="header_container">' + timestamp_topRight + '<div class="primary_name"><a href="#" data-create-user-action-menu="' + user.screen_name + '" class="createUserActionMenu user" screen_name="' + user.screen_name + '">' + user.name + '</a>' + userVerified + userProtected + bothContent + '<a href="#" data-create-user-action-menu="' + user.screen_name + '" class="createUserActionMenu user" screen_name="' + user.screen_name + '">@' + user.screen_name + '</a></div></div>';
    } else if(nameAttribute == "screen_name") {
      header_container = '<div class="header_container"><div class="primary_name"><a href="#" data-create-user-action-menu="' + user.screen_name + '" class="createUserActionMenu user" screen_name="' + user.screen_name + '" title="' + user.name + '">@' + user.screen_name + '</a>' + userVerified + userProtected + '</div></div>';
    } else if(nameAttribute == "name") {
      header_container = '<div class="header_container"><div class="primary_name"><a href="#" data-create-user-action-menu="' + user.screen_name + '" class="createUserActionMenu user" screen_name="' + user.screen_name + '" title="@' + user.screen_name + '">' + user.name + '</a>' + userVerified + userProtected + '</div></div>';
    }
    ...
    if(this.isComplete() && !hiddenFooter) {
      ...
      // from App
      if(tweet.source) {
        var anyClickSource = tweet.source.replace(/href=/i, 'class="handleLink" href="#" data-handle-link-base=');
        footer_content += '<span class="from_app">' + chrome.i18n.getMessage("fromApp_prefix") + anyClickSource + chrome.i18n.getMessage("fromApp_suffix") + '</span>';
      }
      // DM
      ...
      if(tweet.geo) {
        ...
        var href = "http://maps.google.com/maps?q=loc:" + coords[0] + "," + coords[1] + " " + encodeURI("(" + tweet.user.screen_name + ")"), latStr = coords[0] + ',' + coords[1];
        footer_content += '<span class="geo_tag"><a class="handleLink tooltip" data-handle-link-base="' + href + '" data-tooltip-image="http://maps.google.com/maps/api/staticmap?' + $.param({center: latStr, zoom: 15, size: '200x200', maptype: 'roadmap', markers: 'size:small|' + latStr, sensor: false}) + '" href="#"><img src="data:image/gif;base64, "/></a></span>';
      }
      // from list
    ...
    if(this.isComplete()) {
      ...
      if(tweet.user.screen_name == tweetManager.twitterBackend.username()) {
        ...
        newActions_container += '<img class="destroyIcon" title="' + titleString + '" src="img/delete.png" /><br /><div class="rt_confirm destroy">' + confirmString + '<a href="#" class="destroyYes">' + chrome.i18n.getMessage("Yes") + '</a> <a href="#" class="destroyNo">' + chrome.i18n.getMessage("No") + '</a></div>';
      } else {
        ...
        if(tweetManager.isRetweet(tweet)) {
          newActions_container += '<img class="destroyIcon" title="' + chrome.i18n.getMessage("deleteRT") + '" src="img/delete.png" /><br /><div class="rt_confirm destroy">' + chrome.i18n.getMessage("deleteRTConfirm") + '<a href="#" class="destroyYes">' + chrome.i18n.getMessage("Yes") + '</a> <a href="#" class="destroyNo">' + chrome.i18n.getMessage("No") + '</a></div>';
        } else {
          if(templateId != TimelineTemplate.RECEIVED_DMS && templateId != TimelineTemplate.SENT_DMS && !user['protected']) {
            newActions_container += '<img class="retweetIcon" title="' + chrome.i18n.getMessage("Retweet") + '" src="img/rt.png" /><br /><div class="rt_confirm">' + chrome.i18n.getMessage("retweetConfirm") + '<a href="#" class="retweetYes">' + chrome.i18n.getMessage("Yes") + '</a> <a href="#" class="retweetNo">' + chrome.i18n.getMessage("No") + '</a></div>';
          }
        }
        if(!user['protected']) {
    ...
    var tweetSpace = $($.parseHTML('<div class="tweet_space" id="' + tweetspaceId + '"><div class="chromed_bird_tweet tweet" timelineid="' + tweetTimeline + '" tweetid="' + tweet.id + '"><div class="tweet_overlay" style="' + overlayStyle + '"><div class="first_container">' + profile_container + header_container + text_container + footer_container + '</div>' + newActions_container + '</div></div></div>'));
  ...
  toggleInReply: function(tweet, targetId) {
    if(tweet.replyVisible) {
      ...
      var tweetSpace = $('#' + targetId).find("[tweetid='" + tweet.in_reply_to_status_id + "']").parents('.tweet_space');
