    ...
    .always(function(data, status, request){
      ...
      try {
        ...
        $.each(allHeaders, function(index, header) {
          switch( true ) {
            case (/X-Rate-?Limit-Remainingt/i).test(header):
              _this.remainingHitsCount = header.split(": ")[1];
              break;
            case (/X-Rate-?Limit-Reset/i).test(header):
              _this.nextHitsReset = header.split(": ")[1];
              break;
            case (/X-Rate-?Limit-Limit/i).test(header):
              _this.hourlyLimit = header.split(": ")[1];
              break;
            case (/X-Access-Level/i).test(header):
              _this.lastAccessLevel = header.split(": ")[1];
              break;
          }
        });
