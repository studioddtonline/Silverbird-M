"use strict";
class TwitterLib {
  constructor(onAuthenticated, oauthTokenData) {
    if(typeof onAuthenticated === "undefined") {
      throw new TypeError("onAuthenticated is needed");
    }
    Object.defineProperties(this, {
      ...
      "oauthLib": {
        value: new TwitterOAuth(oauthTokenData, (function(self) {
          return () => {self.verifyCredentials(self.onAuthenticated)};
        })(this))
      },
      "snowflakeIdRegexp": {
  ...
  generateOauthHeader(signedData, includeRealm) {
            ...
            switch(request && request.readyState === 4) {
              ...
              case (request.status === 401):
                ...
                switch(true) {
                  ...
                  default:
                    ...
                    console.warn(`${rspObj.request}:${spObj.error}`);
                ...
                switch(true) {
                  ...
                  case (rspObj.code === 187):
                    ...
                    if(/statuses\/update/.test(url) || /direct_messages\/new/.test(url)) {
                      ...
                      fmtError = `${url}:${rspObj.code}:${spObj.message}`;
                    }
              ...
              case (request.status === 503):
                ...
                if(rspObj.code === 130) {
                  ...
                  fmtError = `Service Unavailable:${rspObj.code}:${spObj.message}`;
                }
                console.warn(rspObj.message);
                break;
              default:
  ...
  tweet(callback, msg, replyId, mediaIds) {
    var params = {
      status: msg
    };
    if(replyId) {
...
class TwitterOAuth {
  constructor(oauthTokenData, onAuthenticated) {
    this.user_id = null;
