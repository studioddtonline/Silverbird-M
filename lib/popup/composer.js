var Composer = {
  ...
  confirmDestroy: function() {
    ...
    tweetManager.destroy(function(success, data, status) {
      ...
      if(success || notFound) {
        $(".tweet").find("[tweetid='" + _this.destroyId + "']").parents('.tweet_space').first().hide('blind', { direction: "vertical" });
        if(_this.destroyRT > 0) {
          delete tweetManager.retweets[_this.destroyRT];
          this.destroyRT = 0;
          loadTimeline(true);
        } else {
          for(var tweetId in tweetManager.retweets) {
            if(!tweetManager.retweets.hasOwnProperty(tweetId)) continue;
            if(tweetManager.retweets[tweetId] == _this.destroyId) {
  ...
  destroy: function (node, retweet) {
    ...
    if(retweet) {
      this.destroyRT = $node.attr('tweetid');
      this.destroyId = tweetManager.retweets[this.destroyRT];
    } else {
      this.destroyRT = 0;
  ...
  textareaChanged: function (e) {
    ...
    var stringCount = 0;
    var link = new RegExp('https?://.+?\.[a-zA-Z\d\.\/\?\&\+\=\%]+?', 'g');
    if(link.test(str)) {
      stringCount = new String(str).replace(link, '***********************').length;
    } else {
      stringCount = new String(str).length;
    }
