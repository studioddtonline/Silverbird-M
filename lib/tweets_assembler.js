...
var Renderer = {
  ...
  makeTweetText: function (content) {
    ...
    var result = Renderer.makeSpan('');
    for(var i = 0, j = splited.length; i < j; i++) {
      result.appendChild(Renderer.makeText(splited[i]));
      if(i < j - 1) result.appendChild(document.createElement('br'));
    }
    return result;
  },
  ...
  renderTweet: function (tweet, useColors, nameAttribute) {
    ...
    if(tweet.in_reply_to_status_id) {
      ...
      inReply.appendChild(Renderer.makeText(tweet.in_reply_to_screen_name));
      var replyAction;
    ...
    var profileDiv = Renderer.makeDiv({'class':'profile'});
    first_container.appendChild(profileDiv);
    ...
    var headerDiv = Renderer.makeDiv({'class':'header'});
    first_container.appendChild(headerDiv);
    ...
    if(!Renderer.isNotification()) {
      ...
      var statusLinkSpan = Renderer.makeElem(
        "span",
        {'class': 'timestamp'}
      );
      var statusLink = Renderer.makeElem(
        "a",
        {
          href: statusLinkDst,
          title: Renderer.getTimestampAltText(tweet.created_at)
        },
        Renderer.getTimestampText(tweet.created_at)
      );
      AnyClick.anyClick(statusLink, function() {openTab(statusLinkDst);});
      statusLinkSpan.appendChild(statusLink);
      headerDiv.appendChild(statusLinkSpan);
    }

    var primaryUserName;
    var primaryNameDiv = Renderer.makeDiv({'class': 'primary_name'});
    if(nameAttribute == "both") {
      primaryUserName = Renderer.makeElem("a", {href: '#', 'class':"user", screen_name: user.screen_name, title: ''});
      primaryUserName.appendChild(Renderer.makeText(user.name));
      primaryNameDiv.appendChild(primaryUserName);
      var secondaryNameDiv = Renderer.makeDiv({'class': 'secondary_name'});
      var secondaryUserName = Renderer.makeElem("a", {href: '#', 'class':"user", screen_name: user.screen_name, title: ''});
      secondaryUserName.appendChild(Renderer.makeText("@" + user.screen_name));
      secondaryNameDiv.appendChild(secondaryUserName);
    } else if(nameAttribute == "screen_name") {
      primaryUserName = Renderer.makeElem("a", {href: '#', 'class':"user", screen_name: user.screen_name, title: user.name});
      primaryUserName.appendChild(Renderer.makeText("@" + user.screen_name));
      primaryNameDiv.appendChild(primaryUserName);
    } else if(nameAttribute == "name") {
      primaryUserName = Renderer.makeElem("a", {href: '#', 'class':"user", screen_name: user.screen_name, title: "@" + user.screen_name});
      primaryUserName.appendChild(Renderer.makeText(user.name));
      primaryNameDiv.appendChild(primaryUserName);
    }
    headerDiv.appendChild(primaryNameDiv);
    Renderer.createUserActionMenu(primaryUserName, user.screen_name);
    if(secondaryNameDiv) {
      headerDiv.appendChild(secondaryNameDiv);
      Renderer.createUserActionMenu(secondaryUserName, user.screen_name);
    }

    if(user.verified) {
      primaryNameDiv.appendChild(Renderer.makeText(" "));
      primaryNameDiv.appendChild(Renderer.makeElem("img", {'class':'verified', src:'img/verified.png', alt:'verified'}));
    }
    if(user['protected']) {
      primaryNameDiv.appendChild(Renderer.makeText(" "));
      primaryNameDiv.appendChild(Renderer.makeElem("img", {'class':'protected', src:'img/lock.png', alt:'protected'}));
    }
    ...
    if(Renderer.isNotification()) {
      ...
      AnyClick.anyClick(statusLink, function() {openTab(statusLinkDst);});
      statusLinkSpan.appendChild(statusLink);
      footer.appendChild(statusLinkSpan);
      footer.appendChild(Renderer.makeText(" "));
    }
    if(inReply) {
      footer.appendChild(Renderer.makeText(chrome.i18n.getMessage("inReply_prefix")));
      footer.appendChild(inReply);
      footer.appendChild(Renderer.makeText(chrome.i18n.getMessage("inReply_suffix")));
      footer.appendChild(Renderer.makeText(" "));
    } else if(tweet.retweeted_status) {
      footer.appendChild(Renderer.makeText(chrome.i18n.getMessage("retweetedBy_prefix")));
      ...
      footer.appendChild(Renderer.makeText(chrome.i18n.getMessage("retweetedBy_suffix")));
      footer.appendChild(Renderer.makeText(" "));
    }
    ...
    if(from) {
      ...
      fromSpan.appendChild(Renderer.makeText(chrome.i18n.getMessage("fromApp_prefix")));
      fromSpan.appendChild(from);
      fromSpan.appendChild(Renderer.makeText(chrome.i18n.getMessage("fromApp_suffix")));
    ...
    if(templateId == TimelineTemplate.SENT_DMS) {
      ...
      recipientSpan.appendChild(Renderer.makeText(chrome.i18n.getMessage("sentTo_prefix")));
      recipientSpan.appendChild(recipientUsername);
      recipientSpan.appendChild(Renderer.makeText(chrome.i18n.getMessage("sentTo_suffix")));
    ...
    if(templateId == TimelineTemplate.LISTS && (this.isNotification() || tweetManager.currentTimelineId != tweetTimeline)) {
      ...
      if(list !== null) {
        ...
        (function(path) {
          footer.appendChild(Renderer.makeText(" ("));
          footer.appendChild(Renderer.makeText(chrome.i18n.getMessage("f_footer_list")));
          footer.appendChild(Renderer.makeText(": "));
  ...
  assemblyTweetsOnPage: function (tweets, nameAttribute, fadeTimeout) {
    ...
    if(existingDestination.length == 1) {
      ...
      var destination = $(document.createElement('div')).attr('id', 'chromed_bird_container'),
          controlBar = $(document.createElement('div')).attr('id', 'chromed_bird_control'),
          closeLink = $(document.createElement('a')).attr('id', 'close_trigger').attr('href', '#');
      ...
      controlBar
      .append($(document.createElement('span')).text(' ' + chrome.i18n.getMessage("changeNotificationSettings")))
