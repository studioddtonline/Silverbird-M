function Options() {
  ...
  this.load = function(forceDefault) {
    ...
    var closure_load = function(self) {
      return function() {
        ...
        if(name) {
          ...
          if(converter) {
            ...
            if($this.is('canvas')) {
              paintIcon($this[0], self.optionsMap[name]);
            } else {
    ...
    $('input,select,.color_selector').each(closure_load(this));
  ...
  this.save = function() {
    this.clearErrors();
    var hasErrors = false, closure_save = function(self) {
      return function() {
    ...
    $('input,select').each(closure_save(this));
    ...
    if(!hasErrors) {
      var askForRestart = false, closure_hasErrors = function(self) {
        return function() {
          var $this = $(this), name = $this.attr('name');
          if(name) {
      ...
      $('input,select,.color_selector').each(closure_hasErrors(this));
