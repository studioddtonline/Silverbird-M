  ...
  init: function(force) {
    ...
    tweetManager.lists(force, function(success, lists, status) {
      ...
      tweetManager.eachTimeline(function(timeline) {
        ...
        $listSelect.empty();
        $listSelect.append($("<option>").val(_this.selector_value).text(chrome.i18n.getMessage("selectList")));
        for(var j = 0; j < lists.length; ++j) {
          $listSelect.append($("<option>").val(lists[j].uri).text(lists[j].name));
        }
        $listSelect.append($("<option>").val(_this.update_value).text(chrome.i18n.getMessage("updateLists")));
