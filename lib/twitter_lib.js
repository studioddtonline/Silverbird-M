function TwitterLib(onAuthenticated, onHitsUpdated, microbloggingService, baseUrl, baseOauthUrl, baseSigningUrl, baseOauthSigningUrl, oauthTokenData) {
...
TwitterLib.prototype = {
  ...
  ajaxRequest: function(url, callback, context, requestParams, httpMethod, overriddenTimeout) {
    ...
    .always(function(data, status, request){
      if(!request) return;
      try {
        var remaining = request.getResponseHeader("X-Rate-Limit-Remaining");
        if(remaining) {
          _this.remainingHitsCount = remaining;
          _this.nextHitsReset = request.getResponseHeader("X-Rate-Limit-Reset");
          _this.hourlyLimit = request.getResponseHeader("X-Rate-Limit-Limit");
