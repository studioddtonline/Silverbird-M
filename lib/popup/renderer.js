$.extend(Renderer, {
  assemblyTweets: function (tweets, timelineId) {
    ...
    var renderdText = '', pNow = performance.now(), dNow = Date.now();
    ...
    .on('click.popup', '.new_actions_item', function(event) {
      ...
      switch(true) {
        case (cl.contains('glyphicon-star-empty')):
          Composer.favorite(targetNode);
          break;
        case (cl.contains('glyphicon-star')):
          Composer.unFavorite(targetNode);
          break;
        case (cl.contains('glyphicon-retweet')):
          Composer.retweet(targetNode);
          break;
        case (cl.contains('glyphicon-trash')):
          Composer.destroy(targetNode, false);
          break;
        case (cl.contains('glyphicon-remove-circle')):
          Composer.destroy(targetNode, true);
          break;
        case (cl.contains('glyphicon-reply')):
          Composer.reply(targetNode);
          break;
        case (cl.contains('glyphicon-comment')):
